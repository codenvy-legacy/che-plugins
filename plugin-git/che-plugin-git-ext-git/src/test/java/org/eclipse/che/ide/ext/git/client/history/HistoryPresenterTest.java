...
import static org.eclipse.che.ide.ext.git.shared.DiffRequest.DiffType.RAW;
...
public class HistoryPresenterTest extends BaseTest {
    ...
    public void testOnProjectChangesClickedWhenDiffRequestIsSuccessful() throws Exception {
        ...
        verify(view).setCompareType(anyString());
        verify(service, times(3)).log(eq(rootProjectDescriptor), eq(false), (AsyncRequestCallback<LogResponse>)anyObject());
    }
    ...
    public void testOnRevisionSelectedWhenDiffRequestIsSuccessful() throws Exception {
        ...
        verify(service)
                .diff(eq(rootProjectDescriptor), (List<String>)anyObject(), eq(DiffRequest.DiffType.RAW), anyBoolean(), anyInt(),
