...
public final class ResourceChangeValidator {
	...
	private static org.eclipse.che.core.resources.mapping.ResourceChangeValidator instance;

	/**
	 * Return the singleton change validator.
	 * @return the singleton change validator
	 */
	public static org.eclipse.che.core.resources.mapping.ResourceChangeValidator getValidator() {
		if (instance == null)
			instance = new org.eclipse.che.core.resources.mapping.ResourceChangeValidator();
		return instance;
	}
	...
	private ModelProvider[] getProviders(IResource[] resources) {
		...
		for (int i = 0; i < descriptors.length; i++) {
			...
			try {
				...
				Policy.log(e.getStatus().getSeverity(), NLS.bind("Could not instantiate provider {0}", descriptor.getId()),
						   e); //$NON-NLS-1$
			}
		}
