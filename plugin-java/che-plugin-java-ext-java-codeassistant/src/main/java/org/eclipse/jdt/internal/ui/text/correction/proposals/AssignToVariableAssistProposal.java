...
public class AssignToVariableAssistProposal extends LinkedCorrectionProposal {
	...
	public AssignToVariableAssistProposal(ICompilationUnit cu, int variableKind, ExpressionStatement node, ITypeBinding typeBinding,
										  int relevance) {
	...
	public AssignToVariableAssistProposal(ICompilationUnit cu, SingleVariableDeclaration parameter,
		...
		if (existingFragment == null) {
			...
			setDisplayName(Messages.format(CorrectionMessages.AssignToVariableAssistProposal_assigntoexistingfield_description,
	...
	private ASTRewrite doAddField() {
		...
		Expression expression = isParamToField ? ((SingleVariableDeclaration)fNodeToAssign).getName()
...
//			boolean safeActionsEnabled= Platform.getPreferencesService().getBoolean(JavaPlugin.getPluginId(), saveActionsKey, false, scopes);
		...
		if (/*safeActionsEnabled &&*/
			...
			&&*/ CleanUpOptions.TRUE.equals(PreferenceConstants.getPreference(
				CleanUpPreferenceUtil.SAVE_PARTICIPANT_KEY_PREFIX + CleanUpConstants.VARIABLE_DECLARATIONS_USE_FINAL, project))
