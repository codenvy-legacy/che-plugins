	...
	private void initReturnType(ImportRewrite rewriter) {
		...
		switch (fReturnKind) {
			case ACCESS_TO_LOCAL:
				...
				fReturnType =
						ASTNodeFactory.newType(ast, declaration, rewriter, new ContextSensitiveImportRewriteContext(declaration,
																													rewriter));
			...
			default:
				...
				for (int i= 0; i < localReads.size(); i++) {
					...
					String bindingName= BindingLabelProvider
							.getBindingLabel(binding, BindingLabelProvider.DEFAULT_TEXTFLAGS | JavaElementLabels.F_PRE_TYPE_SIGNATURE);
				...
				String message= MessageFormat
						.format(RefactoringCoreMessages.ExtractMethodAnalyzer_assignments_to_local, new Object[]{affectedLocals.toString()});
