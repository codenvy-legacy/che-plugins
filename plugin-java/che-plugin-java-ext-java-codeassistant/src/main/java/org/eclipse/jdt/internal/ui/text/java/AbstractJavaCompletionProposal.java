    ...
    public void apply(IDocument document, char trigger, int offset) {
            ...
            for (int i = 0; requiredProposals != null && i < requiredProposals.length; i++) {
                ...
                if (requiredProposals[i].getKind() == CompletionProposal.TYPE_REF) {
                    ...
                    ImportCompletionProposal proposal =
                            new ImportCompletionProposal(requiredProposals[i], fInvocationContext, coreProposal.getKind());
        ...
        try {
            ...
            if (isSmartTrigger || trigger == (char)0) {
                ...
                if (trigger == ';' && replacement.charAt(replacement.length() - 1) != ';' &&
                    (referenceOffset >= document.getLength() || document.getChar(referenceOffset) != ';')) {
...
//		return css;
	...
	protected boolean isPrefix(String prefix, String string) {
		...
		return start.equalsIgnoreCase(prefix) || isCamelCaseMatching() && CharOperation
                .camelCaseMatch(prefix.toCharArray(), string.toCharArray());
	}
