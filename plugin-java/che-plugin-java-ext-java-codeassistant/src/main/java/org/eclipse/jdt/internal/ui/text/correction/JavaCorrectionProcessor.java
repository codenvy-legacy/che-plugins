...
public class JavaCorrectionProcessor implements org.eclipse.che.jface.text.quickassist.IQuickAssistProcessor {
...
//		}
		...
		if(contributionId == QUICKFIX_PROCESSOR_CONTRIBUTION_ID) {
			...
			res.add(new ContributedProcessorDescriptor(new AdvancedQuickAssistProcessor(), testMarkerTypes));
		}
...
//	}
	...
	private static abstract class SafeCorrectionProcessorAccess implements ISafeRunnable {
		...
		public void handleException(Throwable exception) {
			if (fMulti == null) {
				fMulti = new MultiStatus(JavaPlugin.ID_PLUGIN, IStatus.OK, CorrectionMessages.JavaCorrectionProcessor_error_status, null);
			}
			fMulti.merge(new Status(IStatus.ERROR, JavaPlugin.ID_PLUGIN, IStatus.ERROR, CorrectionMessages
					.JavaCorrectionProcessor_error_status,
									exception));
		}
	...
	private static class SafeAssistCollector extends SafeCorrectionProcessorAccess {
		...
		public void safeRun(ContributedProcessorDescriptor desc) throws Exception {
			IQuickAssistProcessor curr =
					(IQuickAssistProcessor)desc.getProcessor(fContext.getCompilationUnit(), IQuickAssistProcessor.class);
			if (curr != null) {
