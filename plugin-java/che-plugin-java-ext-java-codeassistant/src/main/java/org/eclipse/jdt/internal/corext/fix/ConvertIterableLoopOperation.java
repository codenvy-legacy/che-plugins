...
public final class ConvertIterableLoopOperation extends ConvertLoopOperation {
	...
	private static final StatusInfo SEMANTIC_CHANGE_WARNING_STATUS= new StatusInfo(IStatus.WARNING,
																				   FixMessages
	...
	@Override
	protected Statement convert(CompilationUnitRewrite cuRewrite, final TextEditGroup group, final LinkedProposalModel positionGroups)
	...
	public final IStatus satisfiesPreconditions() {
		...
		if (JavaModelUtil.is50OrHigher(getJavaProject())) {
			...
			if (updateExpressions.size() == 1) {
				resultStatus= new StatusInfo(IStatus.WARNING, Messages
						.format(FixMessages.ConvertIterableLoopOperation_RemoveUpdateExpression_Warning,
