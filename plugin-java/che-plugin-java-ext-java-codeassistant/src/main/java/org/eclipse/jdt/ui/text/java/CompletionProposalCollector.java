...
public class CompletionProposalCollector extends CompletionRequestor {
     ...
     * {@link org.eclipse.jdt.core.IJavaProject}, no javadoc will be available as
     * {@link ICompletionProposal#getAdditionalProposalInfo() additional info}
     ...
     * are created on. Use
     * {@link CompletionProposalCollector#CompletionProposalCollector(org.eclipse.jdt.core.ICompilationUnit)}
     ...
     * replacing
     * {@linkplain #createJavaCompletionProposal(org.eclipse.jdt.core.CompletionProposal) createJavaCompletionProposal}
     * instead.
     * </p>
     */
    @Override
	 ...
	 * The passed proposal must be of kind {@link org.eclipse.jdt.core.CompletionProposal#METHOD_REF}.
	 *
	 ...
	 * @see org.eclipse.jdt.core.CompletionRequestor#acceptContext(org.eclipse.jdt.core.CompletionContext)
	 */
	 ...
	 * {@linkplain org.eclipse.jdt.core.CompletionRequestor#setIgnored(int, boolean) setIgnored} and
	...
	private void acceptPotentialMethodDeclaration(CompletionProposal proposal) {
		try {
			...
			if (type != null) {
				...
				GetterSetterCompletionProposal
                        .evaluateProposals(type, prefix, completionStart, completionEnd - completionStart, relevance + 2,
                                           fSuggestedMethodNames, fJavaProposals);
				MethodDeclarationCompletionProposal
                        .evaluateProposals(type, prefix, completionStart, completionEnd - completionStart, relevance, fSuggestedMethodNames,
                                           fJavaProposals);
			}
	...
	private IJavaCompletionProposal createAnnotationAttributeReferenceProposal(CompletionProposal proposal) {
		...
		JavaCompletionProposal
                javaProposal= new JavaCompletionProposal(completion, proposal.getReplaceStart(), getLength(proposal), getImage(descriptor), displayString, computeRelevance(proposal));
	...
	private IJavaCompletionProposal createAnonymousTypeProposal(CompletionProposal proposal, JavaContentAssistInvocationContext invocationContext) {
		...
		try {
			...
			JavaCompletionProposal
                    javaProposal= new AnonymousTypeCompletionProposal(fJavaProject, fCompilationUnit, invocationContext, start, length, completion, label, String.valueOf(proposal
	...
	private IJavaCompletionProposal createFieldProposal(CompletionProposal proposal) {
		...
		JavaCompletionProposal
                javaProposal= new JavaCompletionProposal(completion, start, length, image, label, relevance, getContext().isInJavadoc(), getInvocationContext());
		if (fJavaProject != null)
	...
	private IJavaCompletionProposal createFieldWithCastedReceiverProposal(CompletionProposal proposal) {
		...
		JavaCompletionProposal
                javaProposal= new JavaFieldWithCastedReceiverCompletionProposal(completion, start, length, image, label, relevance, getContext().isInJavadoc(), getInvocationContext(), proposal);
		if (fJavaProject != null)
			javaProposal.setProposalInfo(new FieldProposalInfo(fJavaProject, proposal));
...
//		adaptLength(proposal, javadocProposal);
	...
	private IJavaCompletionProposal createMethodDeclarationProposal(CompletionProposal proposal) {
		...
		JavaCompletionProposal
                javaProposal= new OverrideCompletionProposal(fJavaProject, fCompilationUnit, name, paramTypes, start, length, label, String.valueOf(proposal.getCompletion()));
