...
public class CastCorrectionProposal extends LinkedCorrectionProposal {
	...
	private Type getNewCastTypeNode(ASTRewrite rewrite, ImportRewrite importRewrite) {
		...
		ImportRewriteContext context =
				new ContextSensitiveImportRewriteContext((CompilationUnit)fNodeToCast.getRoot(), fNodeToCast.getStartPosition(),
														 importRewrite);
		...
		if (parent instanceof MethodInvocation) {
			...
			if (invocation.getExpression() == node) {
				...
				ITypeBinding[] bindings = ASTResolving
						.getQualifierGuess(node.getRoot(), invocation.getName().getIdentifier(), invocation.arguments(), targetContext);
