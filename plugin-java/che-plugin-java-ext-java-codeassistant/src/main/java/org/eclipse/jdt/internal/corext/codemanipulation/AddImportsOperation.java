...
public class AddImportsOperation implements IWorkspaceRunnable {
	...
	public void run(IProgressMonitor monitor) throws CoreException, OperationCanceledException {
		...
		try {
			...
			CompilationUnit astRoot =
					SharedASTProvider.getAST(fCompilationUnit, SharedASTProvider.WAIT_YES, new SubProgressMonitor(monitor, 1));
			if (astRoot == null)
	...
	private TextEdit evaluateEdits(CompilationUnit root, ImportRewrite importRewrite, int offset, int length, IProgressMonitor monitor)
		...
		if (types.length == 0) {
			fStatus= JavaUIStatus.createError(IStatus.ERROR,
											  Messages.format(CodeGenerationMessages.AddImportsOperation_error_notresolved_message,
															  BasicElementLabels.getJavaElementName(simpleName)), null);
			return null;
		}
	 ...
	 */
	private TypeNameMatch[] findAllTypes(String simpleTypeName, IJavaSearchScope searchScope, SimpleName nameNode, IProgressMonitor monitor) throws
																																			 JavaModelException {
