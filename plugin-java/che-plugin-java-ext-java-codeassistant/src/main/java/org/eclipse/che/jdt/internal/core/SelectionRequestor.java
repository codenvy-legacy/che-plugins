...
/**
 * @see org.eclipse.jdt.internal.codeassist.ISelectionRequestor#acceptError
 */
...
protected IJavaElement findLocalElement(int pos, MethodScope scope) {
	if (scope != null && scope.isLambdaScope()) {
		...
		if (expression != null && expression.resolvedType != null && expression.resolvedType.isValidBinding()) {
            org.eclipse.che.jdt.internal.core.LambdaExpression lambdaElement = LambdaFactory
                    .createLambdaExpression((JavaElement)parent, expression);
 ...
 * unlike {@link Util#findMethod(org.eclipse.jdt.core.IType, char[], String[], boolean)} , this does not require an IType to get
 * the IMethod element.
