...
public class InternalExtendedCompletionContext {
					...
					for (int i = 0; i < fields.length; i++) {
						if (fields[i] instanceof Initializer) {
							...
							if (initializer.block.sourceStart <= astNode.sourceStart &&
								...
								searchVisibleVariablesAndMethods(scope, this.visibleLocalVariables, this.visibleFields, this
																		 .visibleMethods,
																 notInJavadoc);
								break done;
							}
							...
							if (fieldDeclaration.initialization != null && fieldDeclaration.binding != null) {
								...
								if (fieldDeclaration.initialization.sourceEnd > 0) {
									...
									CompletionNodeDetector
											detector = new CompletionNodeDetector(this.assistNode, fieldDeclaration.initialization);
	...
	private TypeBinding getTypeFromSignature(String typeSignature, Scope scope) {
		...
		try {
			...
			assignableTypeBinding = this.lookupEnvironment.getTypeFromTypeSignature(wrapper, typeVariables, this.assistScope
					.enclosingClassScope().referenceContext.binding, null, TypeAnnotationWalker.EMPTY_ANNOTATION_WALKER);
