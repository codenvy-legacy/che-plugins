...
public class MethodInfo extends ClassFileStruct implements IBinaryMethod, Comparable {
    ...
    static AnnotationInfo[] decodeAnnotations(int offset, boolean runtimeVisible, int numberOfAnnotations,
        ...
        for (int i = 0; i < numberOfAnnotations; i++) {
            result[i] =
                    new AnnotationInfo(methodInfo.reference, methodInfo.constantPoolOffsets, readOffset
                                                                                             + methodInfo.structOffset, runtimeVisible,
                                       false);
            ...
            if (CharOperation.equals(attributeName, AttributeNamesConstants.ExceptionsName)) {
                ...
                if (entriesNumber == 0) {
                    ...
                    for (int j = 0; j < entriesNumber; j++) {
                        utf8Offset =
                                this.constantPoolOffsets[u2At(this.constantPoolOffsets[u2At(readOffset)] - this.structOffset + 1)]
                                - this.structOffset;
    ...
    protected void toStringContent(StringBuffer buffer) {
        ...
        buffer.append('{')
                ...
                .append(getSelector()).append(desc).append('}');
    }
    ...
    private void decodeLocalVariableAttribute(int offset, int codeLength) {
        ...
        if (length != 0) {
            ...
            if (this.argumentNamesIndex != this.argumentNames.length) {
                // resize
                System.arraycopy(this.argumentNames, 0, (this.argumentNames = new char[this.argumentNamesIndex][]), 0,
                                 this.argumentNamesIndex);
            }
        }
    }
}
