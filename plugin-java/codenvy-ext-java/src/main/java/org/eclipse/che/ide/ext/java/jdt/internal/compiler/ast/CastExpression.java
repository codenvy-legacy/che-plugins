...
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.lookup.PolymorphicMethodBinding;
...
public class CastExpression extends Expression {
    ...
    public static void checkNeedForEnclosingInstanceCast(BlockScope scope, Expression enclosingInstance,
        ...
        if (castedExpressionType == enclosingInstanceType) {
            ...
            if (memberType == scope.getMemberType(memberType.sourceName(),
    ...
    /**
     * Only complain for identity cast, since other type of casts may be useful: e.g.  ~((~(long) 0) << 32)  is different from: ~((~0) <<
     * 32)
     */
    ...
    public static void checkNeedForArgumentCasts(BlockScope scope, int operator, int operatorSignature, Expression left,
                                                 int leftTypeId, boolean leftIsCast, Expression right, int rightTypeId,
                                                 boolean rightIsCast) {
        if (scope.compilerOptions().getSeverity(CompilerOptions.UnnecessaryTypeCheck) == ProblemSeverities.Ignore) {
            return;
        }
    ...
    private static void checkAlternateBinding(BlockScope scope, Expression receiver, TypeBinding receiverType,
        ...
        if (binding.isConstructor()) {
            ...
            bindingIfNoCast =
                    receiver.isImplicitThis() ? scope.getImplicitMethod(binding.selector, alternateArgumentTypes,
                                                                        fakeInvocationSite)
                                              : scope.getMethod(receiverType, binding.selector, alternateArgumentTypes,
                                                                fakeInvocationSite);
        }
    ...
    public boolean checkUnsafeCast(Scope scope, TypeBinding castType, TypeBinding expressionType, TypeBinding match,
        ...
        if (match == castType) {
            if (!isNarrowing &&
                match == this.resolvedType.leafComponentType()) { // do not tag as unnecessary when recursing through upper bounds
    ...
    /** @see org.eclipse.che.ide.ext.java.jdt.internal.compiler.ast.Expression#localVariableBinding() */
    @Override
    ...
    /** @see org.eclipse.che.ide.ext.java.jdt.internal.compiler.ast.Expression#optimizedBooleanConstant() */
    @Override
    ...
    public TypeBinding resolveType(BlockScope scope) {
        ...
        if (this.expression instanceof MessageSend) {
            ...
            if (methodBinding != null && methodBinding.isPolymorphic()) {
                messageSend.binding =
                        scope.environment().updatePolymorphicMethodReturnType((PolymorphicMethodBinding)methodBinding, castType);
    ...
    /** @see org.eclipse.che.ide.ext.java.jdt.internal.compiler.ast.Expression#setExpectedType(org.eclipse.che.ide.ext.java.jdt.internal.compiler.lookup.TypeBinding) */
    @Override
    ...
    /** @see org.eclipse.che.ide.ext.java.jdt.internal.compiler.ast.Expression#tagAsNeedCheckCast() */
    @Override
