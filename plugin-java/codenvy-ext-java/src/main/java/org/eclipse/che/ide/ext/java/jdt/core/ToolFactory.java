...
import org.eclipse.che.ide.ext.java.jdt.core.formatter.DefaultCodeFormatter;
...
import org.eclipse.che.ide.ext.java.jdt.internal.core.util.PublicScanner;
...
public class ToolFactory {
    ...
    /**
     * This mode is used for formatting new code when some formatter options should not be used. In particular, options that
     * preserve the indentation of comments are not used. In the future, newly added options may be ignored as well.
     * <p>
    ...
    /**
     * This mode is used for formatting existing code when all formatter options should be used. In particular, options that
     * preserve the indentation of comments are used.
     * <p>
     ...
     * level ( {@link JavaCore#COMPILER_COMPLIANCE}) and the target platform ( {@link JavaCore#COMPILER_CODEGEN_TARGET_PLATFORM}).
     * Without these options, it is not possible for the code formatter to know what kind of source it needs to format.
     * </p>
     * <p>
     * Note this is equivalent to <code>createCodeFormatter(options, M_FORMAT_NEW)</code>. Thus some code formatter options may be
     * ignored. See @{link {@link #M_FORMAT_NEW} for more details.
     * </p>
     *
     * @param options
     *         - the options map to use for formatting with the default code formatter. Recognized options are documented on
     *         <code>JavaCore#getDefaultOptions()</code>. If set to <code>null</code>, then use the current settings from
     ...
     * </p>
     * <p>
     * The given mode determines what options should be enabled when formatting the code. It can have the following values:
     * {@link #M_FORMAT_NEW}, {@link #M_FORMAT_EXISTING}, but other values may be added in the future.
     * </p>
     *
     * @param options
     *         the options map to use for formatting with the default code formatter. Recognized options are documented on
     ...
     *         <code>JavaCore#getOptions</code>.
     * @param mode
     *         the given mode to modify the given options.
     * @return an instance of the built-in code formatter
     * @see CodeFormatter
     * @see JavaCore#getOptions()
     * @since 3.3
     ...
     *         if set to <code>false</code>, comments will be silently consumed
     * @param tokenizeWhiteSpace
     *         if set to <code>false</code>, white spaces will be silently consumed,
     * @param assertMode
     *         if set to <code>false</code>, occurrences of 'assert' will be reported as identifiers (
     ...
     *         if set to <code>true</code>, the scanner will record positions of encountered line separator
    ...
    // * @param sourceLevel if set to <code>&quot;1.3&quot;</code> or <code>null</code>, occurrences of 'assert' will be reported
    // as identifiers
     ...
     *         if set to <code>false</code>, white spaces will be silently consumed,
     * @param recordLineSeparator
     *         if set to <code>true</code>, the scanner will record positions of encountered line
     *         separator ends. In case of multi-character line separators, the last character position is considered. These positions
     *         can then be extracted using {@link IScanner#getLineEnds()}. Only non-unicode escape sequences are
     *         considered as valid line separators.
     * @param sourceLevel
     *         if set to <code>&quot;1.3&quot;</code> or <code>null</code>, occurrences of 'assert' will be reported
     *         as identifiers
     *         ({@link ITerminalSymbols#TokenNameIdentifier}), whereas if set to <code>&quot;1.4&quot;</code>, it
     *         would report assert keywords ({@link ITerminalSymbols#TokenNameassert}). Java 1.4 has introduced
     *         a new 'assert' keyword.
    ...
    public static IScanner createScanner(boolean tokenizeComments, boolean tokenizeWhiteSpace,
        ...
        if (complianceLevelValue == 0)
            complianceLevelValue = ClassFileConstants.JDK1_4; // fault-tolerance
        scanner =
                new PublicScanner(tokenizeComments, tokenizeWhiteSpace, false/*nls*/, sourceLevelValue /*sourceLevel*/,
