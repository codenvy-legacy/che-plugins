...
public class LocalVariableBinding extends VariableBinding {
    ...
    public void recordInitializationStartPC(int pc) {
        ...
        if (index == this.initializationPCs.length) {
            System.arraycopy(this.initializationPCs, 0, (this.initializationPCs = new int[this.initializationCount << 2]),
                             0, index);
        }
        ...
        if (this.initializationCount > 0) {
            ...
            for (int i = 0; i < this.initializationCount; i++) {
                ...
                s +=
                        ...
                        + "-" + ((this.initializationPCs[(i << 1) + 1] == -1) ? "?" : String
                                .valueOf(this.initializationPCs[(i << 1) + 1])); //$NON-NLS-2$ //$NON-NLS-1$
            }
            s += "]"; //$NON-NLS-1$
        }
        return s;
