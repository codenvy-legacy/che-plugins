...
public class Javadoc extends ASTNode {
     ...
     *
     * @param visibility
     *         Level of visiblity allowed to see references
     * @param modifiers
     *         modifiers of java element to be seen
    ...
    public ASTNode getNodeStartingAt(int start) {
        ...
        if (this.seeReferences != null) {
            ...
            for (int i = 0; i < length; i++) {
                org.eclipse.che.ide.ext.java.jdt.internal.compiler.ast.Expression expression = this.seeReferences[i];
    ...
    /*
     * @see org.eclipse.che.ide.java.client.internal.compiler.ast.ASTNode#print(int, java.lang.StringBuffer)
     */
    ...
    public void resolve(MethodScope methScope) {
                ...
                if (!methDecl.isConstructor()) {
                    if (overriding && this.seeReferences[i] instanceof JavadocMessageSend) {
                        ...
                        if (messageSend.binding != null && messageSend.binding.isValidBinding()
                            ...
                            if (superType != null && superType.original() != methDecl.binding.declaringClass
                                ...
                                if (methScope.environment().methodVerifier()
                    ...
                    if (allocationExpr.binding != null && allocationExpr.binding.isValidBinding()) {
                        ...
                        ReferenceBinding superType =
                                (ReferenceBinding)methDecl.binding.declaringClass.findSuperTypeOriginatingFrom(allocType);
                        if (superType != null && superType.original() != methDecl.binding.declaringClass) {
        ...
        boolean reportMissing =
                methDecl == null
                || !((overriding && this.inheritedPositions != null) || superRef ||
                     (methDecl.binding.declaringClass != null && methDecl.binding.declaringClass
                                                                                 .isLocalType()));
                ...
                if (param.binding != null && param.binding.isValidBinding()) {
                    ...
                    for (int j = 0; j < maxBindings && !found; j++) {
                        if (bindings[j] == param.binding) {
                            scope.problemReporter().javadocDuplicatedParamTag(param.token, param.sourceStart, param.sourceEnd,
                                                                              methodDecl.binding.modifiers);
                            found = true;
                        }
                    }
            ...
            if (paramTypeParamLength == 0) {
                if (reportMissing) {
                    for (int i = 0, l = typeParametersLength; i < l; i++) {
                        scope.problemReporter().javadocMissingParamTag(parameters[i].name, parameters[i].sourceStart,
    ...
    private void verifyTypeReference(Expression reference, Expression typeReference, Scope scope, boolean source15,
        ...
        if (resolvedType.isValidBinding()) {
            ...
            if (!canBeSeen(scope.problemReporter().options.reportInvalidJavadocTagsVisibility, modifiers)) {
                scope.problemReporter().javadocHiddenReference(typeReference.sourceStart, reference.sourceEnd, scope,
                                                               modifiers);
                return;
            }
            ...
            if (reference != typeReference) {
                if (!canBeSeen(scope.problemReporter().options.reportInvalidJavadocTagsVisibility, resolvedType.modifiers)) {
                    scope.problemReporter().javadocHiddenReference(typeReference.sourceStart, typeReference.sourceEnd,
                                                                   scope, resolvedType.modifiers);
                    return;
                }
            }
