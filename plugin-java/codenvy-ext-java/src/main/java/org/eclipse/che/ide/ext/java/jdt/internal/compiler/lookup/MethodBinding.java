...
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.codegen.ConstantPool;
...
public class MethodBinding extends Binding {
    ...
    /*
     * Returns true if given parameters are compatible with this method parameters. Callers to this method should first check that
     * the number of TypeBindings passed as argument matches this MethodBinding number of parameters
     */
    ...
    MethodBinding asRawMethod(LookupEnvironment env) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (var.boundsCount() <= 1) {
                ...
                arguments[i] =
                        env.createWildcard(null, 0, rawFirstBound, rawOtherBounds,
                                           org.eclipse.che.ide.ext.java.jdt.internal.compiler.ast.Wildcard.EXTENDS);
            }
        }
    ...
    */
    ...
    /*
     * declaringUniqueKey dot selector genericSignature p.X { <T> void bar(X<T> t) } --> Lp/X;.bar<T:Ljava/lang/Object;>(LX<TT;>;)V
     */
    ...
    public MethodBinding findOriginalInheritedMethod(MethodBinding inheritedMethod) {
        ...
        if (inheritedOriginal.declaringClass != superType) {
            ...
            MethodBinding[] superMethods =
                    ((ReferenceBinding)superType).getMethods(inheritedOriginal.selector, inheritedOriginal.parameters.length);
    ...
    public long getAnnotationTagBits() {
        MethodBinding originalMethod = original();
        if ((originalMethod.tagBits & TagBits.AnnotationResolved) == 0
    ...
    /**
     * @return the annotations for each of the method parameters or <code>null></code> if there's no parameter or no annotation at
     *         all.
     */
    ...
    /*
     * Answer true if the receiver is implementing another method in other words, it is overriding and concrete, and overriden
     * method is abstract Only set for source methods
     */
    ...
    public final boolean isMain() {
        if (this.selector.length == 4 && CharOperation.equals(this.selector, TypeConstants.MAIN)
            ...
            && TypeBinding.VOID == this.returnType && this.parameters.length == 1) {
    ...
    /*
     * Answer true if the receiver is overriding another method Only set for source methods
     */
   ...
   /* Answer true if the receiver is visible to the invocationPackage. */
    ...
    public void setAnnotations(AnnotationBinding[] annotations, AnnotationBinding[][] parameterAnnotations,
        ...
        this.declaringClass.storeAnnotationHolder(this,
                                                  AnnotationHolder
                                                          .storeAnnotations(annotations, parameterAnnotations, defaultValue, optionalEnv));
    }
    ...
    /**
     * Returns the method to use during tiebreak (usually the method itself). For generic method invocations, tiebreak needs to use
     * generic method with erasure substitutes.
     */
        ...
        if (this.thrownExceptions != null) {
            if (this.thrownExceptions != Binding.NO_EXCEPTIONS) {
                ...
                for (int i = 0, length = this.thrownExceptions.length; i < length; i++) {
                    if (i > 0)
                        output.append(", "); //$NON-NLS-1$
                    output.append((this.thrownExceptions[i] != null) ? this.thrownExceptions[i].debugName()
