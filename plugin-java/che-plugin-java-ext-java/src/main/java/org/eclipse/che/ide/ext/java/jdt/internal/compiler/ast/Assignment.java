...
public class Assignment extends Expression {
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
        ...
        flowInfo =
                ((Reference)this.lhs).analyseAssignment(currentScope, flowContext, flowInfo, this, false).unconditionalInits();
        int nullStatus = this.expression.nullStatus(flowInfo);
        if (local != null && (local.type.tagBits & TagBits.IsBaseType) == 0) {
            if (nullStatus == FlowInfo.NULL) {
                flowContext.recordUsingNullReference(currentScope, local, this.lhs, FlowContext.CAN_ONLY_NULL
                                                                                    | FlowContext.IN_ASSIGNMENT, flowInfo);
            }
        }
    ...
    public void generateCode(BlockScope currentScope, boolean valueRequired) {
        ...
        // the lhs is responsible to perform the implicitConversion generation for the assignment since optimized for unused local
        // assignment.
    }
    ...
    /**
     * @see org.eclipse.che.ide.ext.java.jdt.internal.compiler.ast.Expression#resolveTypeExpecting(org.eclipse.che.ide.ext.java.jdt.internal.compiler.lookup.BlockScope,
    ...
    public TypeBinding resolveTypeExpecting(BlockScope scope, TypeBinding expectedType) {
        ...
        if (expectedType == TypeBinding.BOOLEAN && lhsType == TypeBinding.BOOLEAN
