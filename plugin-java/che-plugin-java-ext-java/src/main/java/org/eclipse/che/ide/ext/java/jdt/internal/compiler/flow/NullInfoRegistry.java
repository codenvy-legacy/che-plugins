...
public class NullInfoRegistry extends UnconditionalFlowInfo {
     ...
     *
     * @param upstream
     *         - UnconditionalFlowInfo: the flow info before we enter the try block; only definite assignments are
    ...
    public NullInfoRegistry(UnconditionalFlowInfo upstream) {
        ...
        if ((upstream.tagBits & NULL_FLAG_MASK) != 0) {
            ...
            this.nullBit2 =
                    (u1 = upstream.nullBit1) & (u2 = upstream.nullBit2) & (nu3 = ~(u3 = upstream.nullBit3))
            ...
            if (upstream.extra != null) {
                ...
                for (int i = 0; i < length; i++) {
                    this.extra[2 + 1][i] =
                            (u1 = upstream.extra[1 + 1][i]) & (u2 = upstream.extra[2 + 1][i])
                            & (nu3 = ~(u3 = upstream.extra[3 + 1][i])) & (nu4 = ~(u4 = upstream.extra[4 + 1][i]));
     ...
     *
     * @param flowInfo
     *         - FlowInfo: the flow information that this null info registry may mitigate
    ...
    public UnconditionalFlowInfo mitigateNullInfoOf(FlowInfo flowInfo) {
        ...
        m1 = (s1 = source.nullBit1) & (s3 = source.nullBit3) & (s4 = source.nullBit4)
             // prot. non null
        ...
        m3 =
                s1
                & (s2 & (ns3 = ~s3) & (ns4 = ~s4) & (a3 | a4) | (ns2 = ~s2) & s3 & ns4 & (a2 | a4) | ns2 & ns3 & s4
                                                                                                     & (a2 | a3));
        if ((m = (m1 | m2 | m3)) != 0) {
            newCopy = true;
        ...
        if (this.extra != null && source.extra != null) {
            ...
            for (int i = 0; i < length; i++) {
                m1 =
                        (s1 = source.extra[1 + 1][i]) & (s3 = source.extra[3 + 1][i]) & (s4 = source.extra[4 + 1][i])
