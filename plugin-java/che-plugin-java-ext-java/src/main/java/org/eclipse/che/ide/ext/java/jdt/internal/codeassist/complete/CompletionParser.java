...
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.ast.AND_AND_Expression;
...
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.ast.EmptyStatement;
...
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.ast.IntLiteral;
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.ast.LocalDeclaration;
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.ast.MarkerAnnotation;
...
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.ast.OR_OR_Expression;
...
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.ast.PrefixExpression;
...
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.ast.StringLiteral;
...
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.ast.SynchronizedStatement;
...
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.ast.TrueLiteral;
...
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.parser.JavadocParser;
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.parser.RecoveredBlock;
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.parser.RecoveredElement;
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.parser.RecoveredField;
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.parser.RecoveredInitializer;
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.parser.RecoveredLocalVariable;
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.parser.RecoveredMethod;
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.parser.RecoveredType;
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.parser.RecoveredUnit;
...
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.util.HashtableOfObjectToInt;
...
public class CompletionParser extends AssistParser {
    ...
    protected static final int K_BETWEEN_CATCH_AND_RIGHT_PAREN = COMPLETION_PARSER + 4; // whether we are between the keyword
    ...
    protected static final int K_NEXT_TYPEREF_IS_INTERFACE = COMPLETION_PARSER + 6; // whether the next type reference is an
    // interface

    protected static final int K_NEXT_TYPEREF_IS_EXCEPTION = COMPLETION_PARSER + 7; // whether the next type reference is an
    // exception

    protected static final int K_BETWEEN_NEW_AND_LEFT_BRACKET = COMPLETION_PARSER + 8; // whether we are between the keyword 'new'
    // and the following left braket, i.e. '[',
    // '(' or '{'

    protected static final int K_INSIDE_THROW_STATEMENT = COMPLETION_PARSER + 9; // whether we are between the keyword 'throw' and
    // the end of a throw statement

    protected static final int K_INSIDE_RETURN_STATEMENT = COMPLETION_PARSER + 10; // whether we are between the keyword 'return'
    // and the end of a return statement

    protected static final int K_CAST_STATEMENT = COMPLETION_PARSER + 11; // whether we are between ')' and the end of a cast
    // statement
   ...
   /* the following fields are internal flags */
    ...
    private void addPotentialName(char[] potentialVariableName, int start, int end) {
        ...
        if (this.potentialVariableNamesPtr >= length - 1) {
            System.arraycopy(this.potentialVariableNames, 0, this.potentialVariableNames = new char[length * 2][], 0,
                             length);
            System.arraycopy(this.potentialVariableNameStarts, 0, this.potentialVariableNameStarts = new int[length * 2],
                             0, length);
    ...
    protected void attachOrphanCompletionNode() {
         ...
         /* if in context of a method, persists if inside arguments as a type */
            ...
            /* only consider if inside method header */
                if (!recoveredMethod.foundOpeningBrace) {
                    ...
                    if (orphan instanceof TypeReference) {
                        this.currentElement =
                                this.currentElement.parent.add(new CompletionOnFieldType((TypeReference)orphan, true), 0);
                        return;
                    }

                    if (orphan instanceof Annotation) {
                        CompletionOnAnnotationOfType fakeType =
                                new CompletionOnAnnotationOfType(FAKE_TYPE_NAME, this.compilationUnit.compilationResult(),
                                                                 (Annotation)orphan);
             ...
             */
                ...
                if (method != null) {
                    ...
                    if ((methodDecl.bodyStart == methodDecl.sourceEnd + 1) // was missing opening brace
                        && (Util.getLineNumber(orphan.sourceStart, this.scanner.lineEnds, 0, this.scanner.linePtr) == Util
                ...
                if (expression == this.assistNode) {
                    if (this.topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_MEMBER_VALUE_ARRAY_INITIALIZER) {
                        ...
                        if (expression instanceof SingleNameReference) {
                            ...
                            CompletionOnMemberValueName memberValueName =
                                    new CompletionOnMemberValueName(nameReference.token, nameReference.sourceStart,
                                                                    nameReference.sourceEnd);
                        ...
                        if ((index = lastIndexOfElement(K_ATTRIBUTE_VALUE_DELIMITER)) != -1) {
                            ...
                            MemberValuePair memberValuePair =
                                    new MemberValuePair(this.identifierStack[this.identifierPtr--], expression.sourceStart,
        ...
        if (this.currentElement instanceof RecoveredType || this.currentElement instanceof RecoveredMethod) {
            if (this.currentElement instanceof RecoveredType) {
                ...
                if (recoveredType.foundOpeningBrace && this.genericsPtr > -1) {
                    if (this.genericsStack[this.genericsPtr] instanceof TypeParameter) {
                        ...
                        if (detector.containsCompletionNode()) {
                            this.currentElement.add(new CompletionOnMethodTypeParameter(new TypeParameter[]{typeParameter},
            ...
            if ((!isInsideMethod() && !isInsideFieldInitialization())) {
                if (this.genericsPtr > -1 && this.genericsLengthPtr > -1 && this.genericsIdentifiersLengthPtr > -1) {
                    ...
                    for (int i = 0; i <= this.identifierLengthPtr && numberOfIdentifiers > 0; i++) {
                    ...
                    if (this.assistNodeParent != null && this.assistNodeParent instanceof TypeReference) {
                        if (this.currentElement instanceof RecoveredType) {
                            this.currentElement =
                                    this.currentElement.add(new CompletionOnFieldType((TypeReference)this.assistNodeParent, false),
                                                            0);
                        } else {
        ...
        if (this.expressionPtr > -1) {
             ...
             */
                ...
                if (method != null) {
                    AbstractMethodDeclaration methodDecl = method.methodDeclaration;
                    if ((methodDecl.bodyStart == methodDecl.sourceEnd + 1) // was missing opening brace
                        && (Util.getLineNumber(expression.sourceStart, this.scanner.lineEnds, 0, this.scanner.linePtr) == Util
                            .getLineNumber(methodDecl.sourceEnd, this.scanner.lineEnds, 0, this.scanner.linePtr))) {
                        return;
                    }
                }
                if (expression == this.assistNode
                    || (expression instanceof Assignment // https://bugs.eclipse.org/bugs/show_bug.cgi?id=287939
                        ...
                        ((this.expressionPtr > 0 && stackHasInstanceOfExpression(
                                this.expressionStack, this.expressionPtr - 1))
                         // In case of error in compilation unit, expression stack might not have instanceof exp, so try
        ...
        if (this.astPtr > -1 &&
            this.astStack[this.astPtr] instanceof LocalDeclaration) { // https://bugs.eclipse.org/bugs/show_bug.cgi?id=287939
        ...
        if (memberValuePair instanceof CompletionOnMemberValueName) {
            ...
            if (this.astLengthPtr > -1 && (length = this.astLengthStack[this.astLengthPtr--]) > nodesToRemove) {
                if (this.astStack[this.astPtr] instanceof MemberValuePair) {
                    System.arraycopy(this.astStack, (this.astPtr -= length) + 1, memberValuePairs =
                            new MemberValuePair[length - nodesToRemove], 0, length - nodesToRemove);
                }
            }
            annotation =
                    new CompletionOnAnnotationMemberValuePair(typeReference, this.intStack[this.intPtr--], memberValuePairs,
                                                              memberValuePair);
            ...
            if (this.astLengthPtr > -1 && (length = this.astLengthStack[this.astLengthPtr--]) > nodesToRemove) {
                if (this.astStack[this.astPtr] instanceof MemberValuePair) {
                    System.arraycopy(this.astStack, (this.astPtr -= length) + 1, memberValuePairs =
                            new MemberValuePair[length - nodesToRemove + 1], 0, length - nodesToRemove);
                }
            ...
            switch (kind) {
                case K_SELECTOR_QUALIFIER:
                    ...
                    if (selector == THIS_CONSTRUCTOR || selector == SUPER_CONSTRUCTOR) {
                        ExplicitConstructorCall call =
                                new ExplicitConstructorCall((selector == THIS_CONSTRUCTOR) ? ExplicitConstructorCall.This
                ...
                case K_ASSISGNMENT_OPERATOR:
                    if (this.expressionPtr > 0 && this.expressionStack[this.expressionPtr - 1] != null) {
                        ...
                        if (info == EQUAL) {
                            assignment =
                                    new Assignment(this.expressionStack[this.expressionPtr - 1], expression, expression.sourceEnd);
                        } else {
                            assignment =
                                    new CompoundAssignment(this.expressionStack[this.expressionPtr - 1], expression, info,
                ...
                case K_BETWEEN_CASE_AND_COLON:
                    if (this.expressionPtr > 0) {
                        ...
                        CaseStatement caseStatement =
                                new CaseStatement(expression, expression.sourceStart, expression.sourceEnd);
                ...
                case K_INSIDE_FOR_CONDITIONAL: // https://bugs.eclipse.org/bugs/show_bug.cgi?id=253008
                    ForStatement forStatement =
                            new ForStatement(new Statement[0], expression, new Statement[0], new EmptyStatement(
                                    expression.sourceEnd, expression.sourceEnd), false, expression.sourceStart, expression.sourceEnd);
                ...
                case K_BETWEEN_SYNCHRONIZED_AND_RIGHT_PAREN:
                    SynchronizedStatement synchronizedStatement =
                            new SynchronizedStatement(expression, new Block(0), expression.sourceStart, expression.sourceEnd);
                ...
                case K_INSIDE_ASSERT_EXCEPTION:
                    if (info == this.bracketDepth) {
                        AssertStatement assertStatement =
                                new AssertStatement(expression, new TrueLiteral(expression.sourceStart, expression.sourceStart),
        ...
        while (index >= 0) {
            ...
            if (index != -1 && this.elementInfoStack[index] == IF && this.elementObjectInfoStack[index] != null) {
                ...
                if (this.currentElement instanceof RecoveredLocalVariable
                    ...
                    RecoveredLocalVariable recoveredLocalVariable = (RecoveredLocalVariable)this.currentElement;
                    if (recoveredLocalVariable.localDeclaration.initialization == null && statement instanceof Expression
    ...
    /**
     * Checks if the completion is on the dot following an array type, a primitive type or an primitive array type. Returns whether
     * we found a completion node.
     */
    private boolean checkClassLiteralAccess() {
        if (this.identifierLengthPtr >= 1 &&
            this.previousToken == TokenNameDOT) { // (NB: the top id length is 1 and it is for the completion identifier)
            int length;
            ...
            if ((length = this.identifierLengthStack[this.identifierLengthPtr - 1]) < 0) {
                ...
                // typeRef.sourceEnd = typeRef.sourceStart + typeRef.token.length; // NB: It's ok to use the length of the token since
                // it doesn't contain any unicode
    ...
    private boolean checkKeyword() {
        if (this.currentElement instanceof RecoveredUnit) {
            ...
            if ((index = this.indexOfAssistIdentifier()) > -1) {
                ...
                int count = 0;
                if (unit.typeCount == 0 && this.lastModifiers == ClassFileConstants.AccDefault) {
        ...
        if (isInsideMethod()) {
            ...
            if (kind != K_BLOCK_DELIMITER && (index = indexOfAssistIdentifier()) > -1 && this.expressionPtr > -1
                ...
                if (this.identifierStack[ptr].length > 0
                    ...
                    this.assistNode =
                            new CompletionOnKeyword3(this.identifierStack[ptr], this.identifierPositionStack[ptr],
                                                     Keywords.INSTANCEOF);
                    this.lastCheckPoint = this.assistNode.sourceEnd + 1;
                    this.isOrphanCompletionNode = true;
                    return true;
    ...
    /**
     * Checks if the completion is inside a method invocation or a constructor invocation. Returns whether we found a completion
     * node.
     */
    private boolean checkInvocation() {
        ...
        if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_SELECTOR_QUALIFIER
            && ((isEmptyNameCompletion = topExpression == this.assistNode && isEmptyNameCompletion()) // e.g. it is something like
                // "this.fred([cursor]" but it
                // is not something like
                // "this.fred(1 + [cursor]"
                || (isEmptyAssistIdentifier =
                this.indexOfAssistIdentifier() >= 0 &&
                this.identifierStack[this.identifierPtr].length == 0))) { // e.g. it is something like "this.fred(1 [cursor]"
                ...
                if (selectorPtr == THIS_CONSTRUCTOR || selectorPtr == SUPER_CONSTRUCTOR) {
                    ...
                    CompletionOnExplicitConstructorCall call =
                            new CompletionOnExplicitConstructorCall((selectorPtr == THIS_CONSTRUCTOR)
                                                                    ? ExplicitConstructorCall.This : ExplicitConstructorCall.Super);
    ...
    private boolean checkLabelStatement() {
        if (isInsideMethod() || isInsideFieldInitialization()) {
            ...
            CompletionOnBranchStatementLabel statementLabel =
                    new CompletionOnBranchStatementLabel(kind == K_INSIDE_BREAK_STATEMENT
                                                         ? CompletionOnBranchStatementLabel.BREAK
                                                         : CompletionOnBranchStatementLabel.CONTINUE,
                                                         this.identifierStack[this.identifierPtr--], (int)(position >>> 32), (int)position,
                                                         labels);
    ...
    /**
     * Checks if the completion is on a member access (i.e. in an identifier following a dot). Returns whether we found a
     * completion node.
     */
    ...
    private boolean checkNameCompletion() {
      /*
       * We didn't find any other completion, but the completion identifier is on the identifier stack, so it can only be a
       * completion on name. Note that we allow the completion on a name even if nothing is expected (e.g. foo() b[cursor] would
       * be a completion on 'b'). This policy gives more to the user than he/she would expect, but this simplifies the problem. To
       * fix this, the recovery must be changed to work at a 'statement' granularity instead of at the 'expression' granularity as
       * it does right now.
       */
                ...
                switch (this.invocationType) {
                    ...
                    case NO_RECEIVER: // this case occurs with 'bar().foo'
                        if (this.expressionPtr > -1 && this.expressionLengthStack[this.expressionLengthPtr] == 1) {
                            ...
                            System.arraycopy(this.genericsStack, this.genericsPtr + 1, m.typeArguments =
    ...
    /**
     * Checks if the completion is in the context of a method and on the type of one of its arguments Returns whether we found a
     * completion node.
     */
        ...
        if (this.currentElement instanceof RecoveredMethod) {
          ...
          */
            if (this.lastErrorEndPosition <= this.cursorLocation
                && Util.getLineNumber(this.lastErrorEndPosition, this.scanner.lineEnds, 0, this.scanner.linePtr) == Util
                    .getLineNumber(((CompletionScanner)this.scanner).completedIdentifierStart, this.scanner.lineEnds, 0,
    ...
    /**
     * Checks if the completion is in the context of a type and on a type reference in this type. Persists the identifier into a
     * fake field return type Returns whether we found a completion node.
     */
    ...
    private void classHeaderExtendsOrImplements(boolean isInterface) {
        if (this.currentElement != null && this.currentToken == TokenNameIdentifier
            && this.cursorLocation + 1 >= this.scanner.startPosition && this.cursorLocation < this.scanner.currentPosition) {
            this.pushIdentifier();
            int index = -1;
         /* check if current awaiting identifier is the completion identifier */
            if ((index = this.indexOfAssistIdentifier()) > -1) {
            ...
            /* filter out cases where scanner is still inside type header */
                if (!recoveredType.foundOpeningBrace) {
                    ...
                    if (!isInterface) {
                        ...
                        if (count > 0) {
                            CompletionOnKeyword1 completionOnKeyword =
                                    new CompletionOnKeyword1(this.identifierStack[ptr], this.identifierPositionStack[ptr], keywords);
                        ...
                        if (type.superInterfaces == null) {
                            CompletionOnKeyword1 completionOnKeyword =
                                    new CompletionOnKeyword1(this.identifierStack[ptr], this.identifierPositionStack[ptr],
                                                             Keywords.EXTENDS);
                            completionOnKeyword.canCompleteEmptyToken = true;
    ...
    /*
    * Check whether about to shift beyond the completion token. If so, depending on the context, a special node might need to be
    * created and attached to the existing recovered structure so as to be remember in the resulting parsed structure.
    */
    public void completionIdentifierCheck() {
      ...
      /*
       * In some cases, the completion identifier may not have yet been consumed, e.g. int.[cursor] This is because the grammar
       * does not allow any (empty) identifier to follow a base type. We thus have to manually force the identifier to be consumed
       * (that is, pushed).
       */
        if (assistIdentifier() == null &&
            this.currentToken == TokenNameIdentifier) { // Test below copied from CompletionScanner.getCurrentIdentifierSource()
    ...
    protected void consumeCastExpressionWithPrimitiveType() {
        ...
        this.expressionStack[this.expressionPtr] =
        cast =
                new CastExpression(exp = this.expressionStack[this.expressionPtr + 1], castType =
    ...
    protected void consumeCatchFormalParameter() {
        if (this.indexOfAssistIdentifier() < 0) {
            ...
            CompletionOnArgumentName arg =
                    new CompletionOnArgumentName(identifierName, namePositions, type, this.intStack[this.intPtr + 1]
         ...
         /*
          * if incomplete method header, listLength counter will not have been reset, indicating that some arguments are available
          * on the stack
          */
            this.listLength++;
        }
    ...
    */
        ...
        if (isLocalDeclaration || indexOfAssistIdentifier() < 0 || variableIndex != 0) {
            ...
            if (this.currentElement != null) {
                if (!checkKeyword()
                    ...
                    if (!(this.currentElement instanceof RecoveredType)
                        && (this.currentToken == TokenNameDOT || (Util.getLineNumber(type.sourceStart, this.scanner.lineEnds,
    ...
    /*
    * Copy of code from superclass with the following change: If the cursor location is on the field access, then create a
    ...
    */
    ...
    protected void consumeAnnotationName() {
      ...
      /*
       * retrieve identifiers subset and whole positions, the assist node positions should include the entire replaced source.
       */
    ...
    protected void consumeToken(int token) {
        ...
        if (token == TokenNameIdentifier && this.identifierStack[this.identifierPtr] == assistIdentifier()
            && this.currentElement == null && isIndirectlyInsideFieldInitialization()) {
        ...
        if (isInsideMethod() || isInsideFieldInitialization() || isInsideAttributeValue()) {
            switch (token) {
                ...
                case TokenNamenew:
                    ...
                    this.qualifier = this.expressionPtr; // NB: even if there is no qualification, set it to the expression ptr so that
                    // the number of arguments are correctly computed
    ...
    protected void consumeTypeParameterHeader() {
        ...
        CompletionOnKeyword1 keyword =
                new CompletionOnKeyword1(this.identifierStack[this.identifierPtr],
    ...
    public TypeReference createParameterizedQualifiedAssistTypeReference(char[][] previousIdentifiers,
                                                                         TypeReference[][] typeArguments, char[] assistName,
    ...
    public NameReference createSingleAssistNameReference(char[] assistName, long position) {
        ...
        if (!isInsideMethod()) {
            ...
            if (kind == K_BLOCK_DELIMITER && this.previousKind == K_BLOCK_DELIMITER && this.previousInfo == DO) {
                return new CompletionOnKeyword3(assistName, position, Keywords.WHILE);
            } else if (kind == K_BLOCK_DELIMITER && this.previousKind == K_BLOCK_DELIMITER && this.previousInfo == TRY) {
    ...
    public MethodDeclaration parseSomeStatements(int start, int end, int fakeBlocksCount, CompilationUnitDeclaration unit) {
        ...
        if (!this.hasError) {
            ...
            if (this.astLengthPtr > -1 && (length = this.astLengthStack[this.astLengthPtr--]) != 0) {
                System.arraycopy(this.astStack, (this.astPtr -= length) + 1, fakeMethod.statements = new Statement[length],
                                 0, length);
            }
        }
    ...
    */
    protected boolean resumeAfterRecovery() {
        ...
        if (this.assistNode != null) {
         /*
          * if reached [eof] inside method body, but still inside nested type, or inside a field initializer, should continue in
          * diet mode until the end of the method body or compilation unit
          */
            if ((this.scanner.eofPosition == this.cursorLocation + 1)
                && (!(this.referenceContext instanceof CompilationUnitDeclaration)
                    || isIndirectlyInsideFieldInitialization() || this.assistNodeParent instanceof FieldDeclaration
             ...
             * consume extra tokens which were part of the qualified reference // so that the replaced source comprises them as
             * well if (this.assistNode instanceof NameReference){ int oldEof = scanner.eofPosition; scanner.eofPosition =
             * currentElement.topElement().sourceEnd()+1; scanner.currentPosition = this.cursorLocation+1; int token = -1; try {
             * do { // first token might not have to be a dot if (token >= 0 || !this.completionBehindDot){ if ((token =
             * scanner.getNextToken()) != TokenNameDOT) break; } if ((token = scanner.getNextToken()) != TokenNameIdentifier)
             * break; this.assistNode.sourceEnd = scanner.currentPosition - 1; } while (token != TokenNameEOF); } catch
