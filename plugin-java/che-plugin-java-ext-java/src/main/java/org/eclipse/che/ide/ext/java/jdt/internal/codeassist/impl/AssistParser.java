 ...
 *******************************************************************************/
package org.eclipse.che.ide.ext.java.jdt.internal.codeassist.impl;
...
public abstract class AssistParser extends Parser {
    ...
    protected static final int K_ATTRIBUTE_VALUE_DELIMITER = ASSIST_PARSER + 5; // whether we are inside a annotation attribute
    // valuer
    ...
    protected void consumeOpenBlock() {
        ...
        if (this.realBlockPtr >= stackLength) {
            System
                    .arraycopy(this.blockStarts, 0, this.blockStarts = new int[stackLength + StackIncrement], 0, stackLength);
        }
    ...
    protected void consumePackageDeclarationNameWithModifiers() {
       ...
       */
        ...
        if ((length = this.expressionLengthStack[this.expressionLengthPtr--]) != 0) {
            System.arraycopy(this.expressionStack, (this.expressionPtr -= length) + 1, reference.annotations =
    ...
    public abstract NameReference createQualifiedAssistNameReference(char[][] previousIdentifiers, char[] assistName,
                                                                     long[] positions);

    public abstract TypeReference createQualifiedAssistTypeReference(char[][] previousIdentifiers, char[] assistName,
                                                                     long[] positions);

    public abstract TypeReference createParameterizedQualifiedAssistTypeReference(char[][] previousIdentifiers,
                                                                                  TypeReference[][] typeArguments, char[] asistIdentifier,
    ...
    protected TypeReference getAssistTypeReferenceForGenericType(int dim, int identifierLength, int numberOfIdentifiers) {
      /* no need to take action if not inside completed identifiers */
        if (/* (indexOfAssistIdentifier()) < 0 || */(identifierLength == 1 && numberOfIdentifiers == 1)) {
            ...
            TypeReference reference =
                    createParameterizedSingleAssistTypeReference(typeArguments, assistIdentifier(), positions[0]);
        ...
        while (index > 0) {
            ...
            switch (currentIdentifiersLength) {
                ...
                default:
                    ...
                    System.arraycopy(this.identifierStack, this.identifierPtr + 1, tokens, index - currentIdentifiersLength,
        ...
        if (realLength == 0) {
            if (typeArguments[0] != null && typeArguments[0].length > 0) {
                reference =
                        createParameterizedSingleAssistTypeReference(typeArguments[0], assistIdentifier(), positions[0]);
            } else {
                reference = createSingleAssistTypeReference(assistIdentifier(), positions[0]);
            }
        } else {
            ...
            if (isParameterized || (assistTypeArguments != null && assistTypeArguments.length > 0)) {
                reference =
                        createParameterizedQualifiedAssistTypeReference(tokens, typeArguments, assistIdentifier(),
                                                                        assistTypeArguments, positions);
            } else {
    ...
    /*
     * Copy of code from superclass with the following change: In the case of qualified name reference if the cursor location is on
     * the qualified name reference, then create a CompletionOnQualifiedNameReference instead.
     */
    protected NameReference getUnspecifiedReferenceOptimized() {
      ...
      /*
       * retrieve identifiers subset and whole positions, the completion node positions should include the entire replaced source.
       */
        int length = this.identifierLengthStack[this.identifierLengthPtr];
    ...
    /*
     * Retrieve a partial subset of a qualified name reference up to the completion point. It does not pop the actual awaiting
     * identifiers, so as to be able to retrieve position information afterwards.
     */
    protected char[][] identifierSubSet(int subsetLength) {
        ...
        char[][] subset;
        System.arraycopy(this.identifierStack, this.identifierPtr - this.identifierLengthStack[this.identifierLengthPtr]
                                               + 1, (subset = new char[subsetLength][]), 0, subsetLength);
        return subset;
    }
    ...
    /*
     * Iterate the most recent group of awaiting identifiers (grouped for qualified name reference (e.g. aa.bb.cc) so as to check
     * whether one of them is the assist identifier. If so, then answer the index of the assist identifier (0 being the first
     * identifier of the set). e.g. aa(0).bb(1).cc(2) If no assist identifier was found, answers -1.
     */
    ...
    protected void pushOnElementStack(int kind, int info, Object objectInfo) {
        ...
        if (++this.elementPtr >= stackLength) {
            System.arraycopy(this.elementKindStack, 0, this.elementKindStack = new int[stackLength + StackIncrement], 0,
                             stackLength);
            System.arraycopy(this.elementInfoStack, 0, this.elementInfoStack = new int[stackLength + StackIncrement], 0,
                             stackLength);
            System.arraycopy(this.elementObjectInfoStack, 0, this.elementObjectInfoStack =
                    new Object[stackLength + StackIncrement], 0, stackLength);
        }
        ...
        switch (this.currentToken) {
            ...
            case TokenNameRBRACE:
                ...
                if (this.currentElement != oldElement && !isInsideAttributeValue()) {
                    if (oldElement instanceof RecoveredInitializer || oldElement instanceof RecoveredMethod
                        ...
                        popUntilElement(K_TYPE_DELIMITER);
                        if (!(this.referenceContext instanceof CompilationUnitDeclaration)
                            || isIndirectlyInsideFieldInitialization() || this.currentElement instanceof RecoveredUnit) {
                            popElement(K_TYPE_DELIMITER);
                        }
                    }
                }
    ...
    /**
     * If the given ast node is inside an explicit constructor call then wrap it with a fake constructor call. Returns the wrapped
     * completion node or the completion node itself.
     */
    protected ASTNode wrapWithExplicitConstructorCallIfNeeded(ASTNode ast) {
        ...
        if (ast != null && topKnownElementKind(ASSIST_PARSER) == K_SELECTOR && ast instanceof Expression
            && (((selector = topKnownElementInfo(ASSIST_PARSER)) == THIS_CONSTRUCTOR) || (selector == SUPER_CONSTRUCTOR))) {
