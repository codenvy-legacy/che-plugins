...
public abstract class AbstractCommentParser implements JavadocTagConstants {
        ...
        try {
                        ...
                        if ((!this.lineStarted || previousChar == '{')) {
                            if (this.inlineTagStarted) {
                                ...
                                if (this.reportProblems) {
                                    int end =
                                            previousPosition < invalidInlineTagLineEnd ? previousPosition : invalidInlineTagLineEnd;
            ...
            if (this.inlineTagStarted) {
                ...
                setInlineTagStarted(false);
                       ...
                       && (this.textStart < this.starPosition || this.starPosition == lastStarPosition)) {
                pushText(this.textStart, textEndPosition);
            }
            updateDocComment();
        } catch (Exception ex) {
     ...
     *         The position of the '<' character on which the tag might start
     * @param endTextPosition
     *         The position of the end of the previous text
                                ...
                                while (this.index < this.javadocEnd) { // main loop to search for the </a> pattern
                                    ...
                                    while (readToken() != TerminalTokens.TokenNameGREATER) {
                                        if (this.scanner.currentPosition >= this.scanner.eofPosition
                                            ...
                                            // Signal syntax error
                                            if (this.tagValue !=
                                                TAG_VALUE_VALUE) { // do not report error for @value tag, this will be done after...
                                                if (this.reportProblems)
    ...
    protected boolean parseIdentifierTag(boolean report) {
        ...
        if (report) {
            this.sourceParser.problemReporter().javadocMissingIdentifier(this.tagSourceStart, this.tagSourceEnd,
        ...
        if (readToken() == TerminalTokens.TokenNameIdentifier) {
            ...
            if (readToken() == TerminalTokens.TokenNameLPAREN) {
                ...
                try {
                    ...
                    int end =
                            this.scanner.getCurrentTokenEndPosition() < this.lineEnd ? this.scanner.getCurrentTokenEndPosition()
    ...
    protected boolean parseParam() throws InvalidInputException {
        ...
        try {
            ...
            if (this.reportProblems) {
                ...
                try {
                    while ((token = readToken()) != TerminalTokens.TokenNameWHITESPACE
    ...
    protected boolean parseReference() throws InvalidInputException {
        ...
        try {
            ...
            while (this.index < this.scanner.eofPosition) {
                ...
                switch (token) {
                    case TerminalTokens.TokenNameStringLiteral: // @see "string"
                        ...
                        if (this.reportProblems)
                            this.sourceParser.problemReporter().javadocUnexpectedText(this.scanner.currentPosition,
                                                                                      this.lineEnd);
                        return false;
                        ...
                        if (currentError.length > 0 && currentError[0] == '"') {
                            if (this.reportProblems) {
                                ...
                                if (this.tagValue == TAG_SEE_VALUE) {
                                    ...
                                    if (i < (length - 2) && currentError[i] == ':' && currentError[i + 1] == '/'
                                        && currentError[i + 2] == '/') {
                                        isUrlRef = true;
                                    }
                                }
                                if (isUrlRef) {
                                    ...
                                    this.sourceParser.problemReporter().javadocInvalidSeeUrlReference(
                                            this.scanner.getCurrentTokenStartPosition(), getTokenEndPosition());
                                } else {
                                    this.sourceParser.problemReporter().javadocInvalidReference(
                                            this.scanner.getCurrentTokenStartPosition(), getTokenEndPosition());
                                }
                            }
                            return false;
            ...
            switch (ch) {
                ...
                case '(':
                    if (this.reportProblems)
                        this.sourceParser.problemReporter().javadocMissingHashCharacter(typeRefStartPosition, this.lineEnd,
                                                                                        String.valueOf(this.source, typeRefStartPosition,
                                                                                                       this.lineEnd - typeRefStartPosition +
                                                                                                       1));
                    return false;
                ...
                case ':':
                    ...
                    if (ch == '/' && ch == readChar()) {
                        if (this.reportProblems) {
                            this.sourceParser.problemReporter().javadocInvalidSeeUrlReference(typeRefStartPosition,
                                                                                              this.lineEnd);
                            return false;
                        }
                    }
    ...
    protected boolean parseThrows() {
        ...
        try {
            ...
            if (typeRef == null) {
                if (this.reportProblems)
                    this.sourceParser.problemReporter().javadocMissingThrowsClassName(this.tagSourceStart,
    ...
    protected char peekChar() {
        ...
        if (c == '\\' && this.source[idx] == 'u') {
            ...
            if (!(((c1 = ScannerHelper.getNumericValue(this.source[idx++])) > 15 || c1 < 0)
                  ...
                  || ((c3 = ScannerHelper.getNumericValue(this.source[idx++])) > 15 || c3 < 0) || ((c4 =
                    ScannerHelper.getNumericValue(this.source[idx++])) > 15 || c4 < 0))) {
    ...
    protected void pushOnAstStack(Object node, boolean newLength) {
        ...
        if (node == null) {
            ...
            if (++this.astLengthPtr >= stackLength) {
                System.arraycopy(this.astLengthStack, 0, this.astLengthStack = new int[stackLength + AST_STACK_INCREMENT],
                                 0, stackLength);
            }
    ...
    protected void recordInheritedPosition(long position) {
        if (this.inheritedPositions == null) {
            ...
            if (this.inheritedPositionsPtr == this.inheritedPositions.length) {
                System.arraycopy(this.inheritedPositions, 0, this.inheritedPositions =
                        new long[this.inheritedPositionsPtr + INHERITED_POSITIONS_ARRAY_INCREMENT], 0,
