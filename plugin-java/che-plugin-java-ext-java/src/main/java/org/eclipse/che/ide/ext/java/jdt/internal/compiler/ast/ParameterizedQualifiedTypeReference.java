 ...
 *     Stephan Herrmann - Contribution for Bug 342671 - ClassCastException: org.eclipse.che.ide.java.client.internal.compiler.lookup
 *     .SourceTypeBinding cannot be cast to org.eclipse.che.ide.java.client.internal.compiler.lookup.ArrayBinding
 *******************************************************************************/
...
public class ParameterizedQualifiedTypeReference extends ArrayQualifiedTypeReference {
    ...
    /* (non-Javadoc)
      * @see org.eclipse.che.ide.java.client.internal.compiler.ast.ArrayQualifiedTypeReference#getTypeBinding(org.eclipse.che.ide.java.client
      * .internal.compiler.lookup.Scope)
      */
    @Override
        ...
        for (int i = packageBinding == null ? 0 : packageBinding.compoundName.length, max = this.tokens.length; i < max; i++) {
                ...
                if (qualifyingType != null) {
                    qualifyingType =
                            currentType.isStatic() ? (ReferenceBinding)scope.environment()
                                                                            .convertToRawType(qualifyingType,
                                                                                              false /*do not force conversion of
                                                                                              enclosing types*/)
                                                   : scope
            ...
            if (args != null) {
                ...
                if (typeVariables == Binding.NO_TYPE_VARIABLES) { // check generic
                    if (scope.compilerOptions().originalSourceLevel >=
                        ClassFileConstants.JDK1_5) { // below 1.5, already reported as syntax error
                ...
                if (typeIsConsistent && !currentType.isStatic()) {
                    ...
                    if (actualEnclosing != null && actualEnclosing.isRawType()) {
                        scope.problemReporter().rawMemberTypeCannotBeParameterized(this,
                                                                                   scope.environment()
                                                                                        .createRawType(currentOriginal, actualEnclosing),
                                                                                   argTypes);
                        typeIsConsistent = false;
                    }
                }
                ...
                if (currentOriginal.isGenericType()) {
                    if (typeIsConsistent && qualifyingType != null && qualifyingType.isParameterizedType()) {
                        scope.problemReporter().parameterizedMemberTypeMissingArguments(this,
                                                                                        scope.environment()
                                                                                             .createParameterizedType(currentOriginal, null,
                                                                                                                      qualifyingType), i);
                        typeIsConsistent = false;
                    }
