...
public class InternalNamingConventions {
    ...
    private static char[][] computeNonBaseTypeNames(char[] sourceName, boolean isConstantField, boolean onlyLongest) {
        ...
        int previousCharKind =
                ScannerHelper.isLowerCase(c) ? IS_LOWER_CASE : ScannerHelper.isUpperCase(c) ? IS_UPPER_CASE : c == '_'
                                                                                                              ? IS_UNDERSCORE : IS_OTHER;
                ...
                case IS_UPPER_CASE:
                    if (previousCharKind == IS_LOWER_CASE) {
                        ...
                        if (i > 0) {
                            ...
                            previousCharKind =
                                    ScannerHelper.isLowerCase(pc) ? IS_LOWER_CASE : ScannerHelper.isUpperCase(pc) ? IS_UPPER_CASE
                                                                                                                  : pc == '_'
                                                                                                                    ? IS_UNDERSCORE
                                                                                                                    : IS_OTHER;
                        }
                        ...
                        case IS_UNDERSCORE:
                            ...
                            if (isConstantField) {
                                if (i > 0) {
                                    char pc = sourceName[i - 1];
                                    previousCharKind =
                                            ScannerHelper.isLowerCase(pc) ? IS_LOWER_CASE : ScannerHelper.isUpperCase(pc)
                                                                                            ? IS_UPPER_CASE
    ...
    public static char[] getBaseName(int variableKind, char[] name, boolean updateFirstCharacter) {
    ...
    public static char[] removeVariablePrefixAndSuffix(int variableKind, char[] name) {
        AssistOptions assistOptions;
    ...
    public static void suggestVariableNames(int variableKind, int baseNameKind, char[] baseName, int dim,
                                            char[] internalPrefix, char[][] excluded, boolean evaluateDefault, INamingRequestor requestor) {
        ...
        for (int i = 0; i < tempNames.length; i++) {
            ...
            if (!isConstantField) {
                ...
                for (int j = 0; j <= internalPrefix.length; j++) {
                    if (j == internalPrefix.length
                        || CharOperation
                            .prefixEquals(CharOperation.subarray(internalPrefix, j, -1), unprefixedName, j != 0 /*
                                                                                                          * do not check case when
                                                                                                          * there is no prefix
                                                                                                          */)) {
                        matchingIndex = j;
                        break done;
            ...
            if (matchingIndex > -1) {
                if (!isConstantField) {
                    tempName =
                            CharOperation.concat(CharOperation.subarray(internalPrefix, 0, matchingIndex), unprefixedName);
                    if (matchingIndex == 0)
                                ...
                                case TerminalTokens.TokenNameIdentifier:
                                    ...
                                    if (token == TerminalTokens.TokenNameEOF && nameScanner.startPosition == suffixName.length) {
                                        if (!foundNames.includes(suffixName)) {
                                            acceptName(suffixName, prefixes[k], suffixes[l], k == 0, l == 0, internalPrefix.length
                                                                                                             - matchingIndex, requestor);
