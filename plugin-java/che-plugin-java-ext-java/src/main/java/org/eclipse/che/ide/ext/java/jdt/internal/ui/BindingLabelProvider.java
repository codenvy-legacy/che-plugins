...
public class BindingLabelProvider {
        ...
        if ((flags & JavaElementLabels.F_FULLY_QUALIFIED) != 0) {
            ...
            if (declaringClass != null) { // test for array.length
                getTypeLabel(declaringClass,
                             JavaElementLabels.T_FULLY_QUALIFIED | (flags & JavaElementLabels.P_COMPRESSED), buffer);
                buffer.append('.');
            }
        }
    ...
    private static void getMethodLabel(IMethodBinding binding, long flags, StringBuffer buffer) {
        ...
        if ((flags & JavaElementLabels.M_FULLY_QUALIFIED) != 0) {
            getTypeLabel(binding.getDeclaringClass(), JavaElementLabels.T_FULLY_QUALIFIED
                                                      | (flags & JavaElementLabels.P_COMPRESSED), buffer);
            buffer.append('.');
        }
        buffer.append(binding.getName());
    ...
    private static void getTypeLabel(ITypeBinding binding, long flags, StringBuffer buffer) {
        ...
        if ((flags & (JavaElementLabels.T_FULLY_QUALIFIED | JavaElementLabels.T_CONTAINER_QUALIFIED)) != 0) {
            ...
            if (declaring != null) {
                getTypeLabel(declaring, JavaElementLabels.T_CONTAINER_QUALIFIED | (flags & JavaElementLabels.P_COMPRESSED),
                             buffer);
                buffer.append('.');
            }
        ...
        if ((flags & JavaElementLabels.T_POST_QUALIFIED) != 0) {
            ...
            if (declaringMethod != null) {
                buffer.append(JavaElementLabels.CONCAT_STRING);
                getMethodLabel(declaringMethod, JavaElementLabels.T_FULLY_QUALIFIED
                                                | (flags & JavaElementLabels.P_COMPRESSED), buffer);
            } else if (declaringType != null) {
                buffer.append(JavaElementLabels.CONCAT_STRING);
                getTypeLabel(declaringType, JavaElementLabels.T_FULLY_QUALIFIED | (flags & JavaElementLabels.P_COMPRESSED),
                             buffer);
            } else {
                final IPackageBinding pack = binding.getPackage();
                if (pack != null && !pack.isUnnamed()) {
     ...
     * @param flags
     *         The text flags as defined in {@link JavaElementLabels}
    ...
    /**
     * @param textFlags
     *         Flags defined in {@link JavaElementLabels}.
     * @param imageFlags
     *         Flags defined in {@link JavaElementImageProvider}.
     */
