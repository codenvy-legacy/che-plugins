...
public class FormatterCommentParser extends JavadocParser implements IJavaDocTagConstants {
    ...
    protected void createTag() {
        ...
        if (this.inlineTagStarted) {
            FormatJavadocBlock block =
                    new FormatJavadocBlock(this.inlineTagStart, this.tagSourceEnd, lineStart, this.tagValue);
            ...
            FormatJavadocBlock block =
                    new FormatJavadocBlock(this.tagSourceStart, this.tagSourceEnd, lineStart, this.tagValue);
        ...
        try {
            ...
            switch (token) {
                case TerminalTokens.TokenNameIdentifier:
                    ...
                    if (htmlIndex == JAVADOC_TAGS_ID_MASK)
                        return false;
                    if (htmlPtr >= 0) {
                        ...
                        if (this.htmlTagsPtr == -1 || !CharOperation.equals(this.htmlTags[this.htmlTagsPtr], htmlTag, false)) {
                            if (++this.htmlTagsPtr == 0) { // lazy initialization
                                ...
                                if (this.htmlTagsPtr == this.htmlTags.length) {
                                    System.arraycopy(this.htmlTags, 0, (this.htmlTags =
                                            new char[this.htmlTags.length + AST_STACK_INCREMENT][]), 0, this.htmlTagsPtr);
                                }
                            }
                ...
                if (length == TAG_LINK_LENGTH && CharOperation.equals(TAG_LINK, tagName)) {
                    ...
                    if (this.inlineTagStarted) {
                        ...
                        if (this.reportProblems) {
                           ...
                           && CharOperation.equals(TAG_LITERAL, tagName)) {
    ...
    protected boolean pushParamName(boolean isTypeParam) {
        ...
        FormatJavadocBlock block =
                new FormatJavadocBlock(this.tagSourceStart, this.tagSourceEnd, lineTagStart, TAG_PARAM_VALUE);
    ...
    protected boolean pushSeeRef(Object statement) {
        ...
        int lineTagStart = this.scanner.getLineNumber(this.tagSourceStart);
        FormatJavadocBlock block =
                new FormatJavadocBlock(this.tagSourceStart, this.tagSourceEnd, lineTagStart, this.tagValue);
        block.reference = reference;
        block.sourceEnd = reference.sourceEnd;
        if (this.inlineTagStarted) {
    ...
    private void pushText(int start, int end, int htmlIndex, int htmlDepth) {
        ...
        // Add the text
        FormatJavadocText text =
                new FormatJavadocText(start, end - 1, lineStart, htmlIndex, htmlDepth == -1 ? 0 : htmlDepth);
