 ...
 * </pre>
 * <p/>
 * If, however, the engine is unable to offer completion proposals for whatever reason, <code>completionFailure</code> is called
 ...
 * </pre>
 * <p/>
 * In either case, the bracketing <code>beginReporting</code> <code>endReporting</code> calls are always made as well as
...
public abstract class CompletionRequestor {
    ...
    /**
     * The set of CompletionProposal kinds that this requestor ignores; <code>0</code> means the set is empty. 1 <<
    ...
    /**
     * The set of CompletionProposal kinds that this requestor allows for required proposals; <code>0</code> means the set is
     * empty. 1 << completionProposalKind
     */
    ...
    /**
     * Creates a new completion requestor. If <code>ignoreAll</code> is <code>true</code> the requestor is not interested in all
     * kinds of completion proposals; all will be ignored. For each kind of completion proposals that is of interest,
     * <code>setIgnored(kind, false)</code> must be called. If <code>ignoreAll</code> is <code>false</code> the requestor is
     * interested in all kinds of completion proposals; none will be ignored.
     *
     * @param ignoreAll
     *         <code>true</code> to ignore all kinds of completion proposals, and <code>false</code> to propose all kinds
     * @since 3.4
     */
     ...
     *         one of the kind constants declared on <code>CompletionProposal</code>
     * @return <code>true</code> if the given kind of completion proposal is ignored by this requestor, and <code>false</code> if
     *         it is of interest
     ...
     *         one of the kind constants declared on <code>CompletionProposal</code>
     * @param ignore
     *         <code>true</code> if the given kind of completion proposal is ignored by this requestor, and
     *         <code>false</code> if it is of interest
     * @see #isIgnored(int)
     * @see CompletionProposal#getKind()
     */
     ...
     * @param requiredProposalKind
     *         one of the kind constants declared on <code>CompletionProposal</code>
     * @return <code>true</code> if a proposal of a given kind with a required proposal of the given kind is allowed by this
     *         requestor, and <code>false</code> if it isn't of interest.
     *         <p>
     ...
     * <p/>
     * A required proposal of a given kind is proposed even if {@link #isIgnored(int)} return <code>true</code> for that kind.
     * <p/>
     * Currently only a subset of kinds support required proposals. To see what combinations are supported you must look at
     * {@link CompletionProposal#getRequiredProposals()} documentation.
     *
     * @param proposalKind
     *         one of the kind constants declared
     * @param requiredProposalKind
     *         one of the kind constants declared on <code>CompletionProposal</code>
     * @param allow
     *         <code>true</code> if a proposal of a given kind with a required proposal of the given kind is allowed by this
     *         requestor, and <code>false</code> if it isn't of interest
     ...
     * e.g. <code>{"java.util.Arrays.*"}</code> It can be a reference to a static method or field (as in a static import)<br>
    ...
    /**
     * Set the favorite references which will be used to compute some completion proposals. A favorite reference is a qualified
    ...
    /**
     * Proposes a completion. Has no effect if the kind of proposal is being ignored by this requestor. Callers should consider
     * checking {@link #isIgnored(int)} before avoid creating proposal objects that would only be ignored.
     * <p/>
     * Similarly, implementers should check {@link #isIgnored(int) isIgnored(proposal.getKind())} and ignore proposals that have
     * been declared as uninteresting. The proposal object passed is only valid for the duration of completion operation.
     *
     * @param proposal
     ...
     * <p>
     * This method is called one and only one time before any call to {@link #accept(CompletionProposal)}. The default
     * implementation of this method does nothing. Clients may override.
     * </p>
     *
     * @param context
     ...
     *
     * @param require
     *         <code>true</code> if this requestor requires an extended context.
     * @see CompletionContext#isExtended()
     * @since 3.4
     */
