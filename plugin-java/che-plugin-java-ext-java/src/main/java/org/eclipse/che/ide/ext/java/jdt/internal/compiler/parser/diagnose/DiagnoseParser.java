 ...
 *******************************************************************************/
package org.eclipse.che.ide.ext.java.jdt.internal.compiler.parser.diagnose;
...
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.parser.ParserBasicInformation;
...
public class DiagnoseParser implements ParserBasicInformation, TerminalTokens {
    ...
    public DiagnoseParser(Parser parser, int firstToken, int start, int end, int[] intervalStartToSkip,
                          int[] intervalEndToSkip, int[] intervalFlagsToSkip, CompilerOptions options) {
        this.parser = parser;
        this.options = options;
        this.lexStream =
                new LexStream(BUFF_SIZE, parser.scanner, intervalStartToSkip, intervalEndToSkip, intervalFlagsToSkip,
        ...
        try {
            ...
            do {
                ...
                while (act > ERROR_ACTION || act < ACCEPT_ACTION) { // SHIFT-REDUCE action or SHIFT action ?
                    ...
                    while (act <= NUM_RULES) {
                        ...
                        do {
                            ...
                            this.tempStackTop -= (Parser.rhs[act] - 1);
                            act =
                                    (this.tempStackTop > next_pos ? this.tempStack[this.tempStackTop]
                ...
                if (act == ERROR_ACTION) {
                    ...
                    if (this.parser.problemReporter().options.maxProblemsPerUnit <
    ...
    private RepairCandidate primaryPhase(int error_token) {
        ...
        //
        if (repair.code == INVALID_CODE || repair.code == DELETION_CODE || repair.code == SUBSTITUTION_CODE
    ...
    private PrimaryRepairInfo checkPrimaryDistance(int stck[], int stack_top, PrimaryRepairInfo repair) {
        ...
        //
        j =
                parseCheck(stck, stack_top, this.lexStream.kind(this.buffer[repair.bufferPosition + 1]),
        ...
        while (act <= NUM_RULES) {
            ...
            do {
                ...
                if (this.tempStackTop < threshold) {
                    return (highest_symbol > NT_OFFSET ? Parser.non_terminal_index[highest_symbol - NT_OFFSET]
    ...
    private void scopeTrialCheck(int stck[], int stack_top, PrimaryRepairInfo repair, int indx) {
        ...
        for (int i = 0; i < SCOPE_SIZE; i++) {
            ...
            if (act != ERROR_ACTION) {
                ...
                for (j = this.tempStackTop + 1; j >= (max_pos + 1)
                ...
                if (Parser.scope_rhs[k] == 0 && j < marked_pos) { // match?
                    ...
                    for (j = Parser.scope_state_set[i]; stck[stack_position] != Parser.scope_state[j]
                                                        && Parser.scope_state[j] != 0; j++) {/* empty */
                    }
                    //
    ...
    private RepairCandidate secondaryPhase(int error_token) {
        ...
        for (last_index = MAX_DISTANCE - 1; last_index >= 1
                                            && this.lexStream.kind(this.buffer[last_index]) == EOFT_SYMBOL; last_index--) {/* empty */
        }
        last_index++;
        ...
        if (repair.code == SECONDARY_CODE || repair.code == DELETION_CODE) {
            ...
            for (scope_repair.bufferPosition = 2; scope_repair.bufferPosition <= repair.bufferPosition
    ...
    private void secondaryDiagnosis(SecondaryRepairInfo repair) {
        switch (repair.code) {
            case SCOPE_CODE: {
                ...
                for (int i = 0; i < this.scopeStackTop; i++) {
                    reportError(SCOPE_CODE, -this.scopeIndex[i], this.locationStack[this.scopePosition[i]], this.buffer[1],
                                Parser.non_terminal_index[Parser.scope_lhs[this.scopeIndex[i]]]);
                }
            ...
            default: {
                reportError(
                        repair.code,
                        (repair.code == SECONDARY_CODE ? getNtermIndex(this.stack[repair.stackPosition], repair.symbol,
                                                                       repair.bufferPosition) : Parser.terminal_index[ERROR_SYMBOL]),
        ...
        switch (msgCode) {
            case BEFORE_CODE:
                ...
                if (this.reportProblem)
                    problemReporter().parseErrorInsertBeforeToken(errorStart, errorEnd, currentKind, errorTokenSource,
                                                                  errorTokenName, name);
                break;
            case INSERTION_CODE:
                if (this.recoveryScanner != null) {
                ...
                if (this.reportProblem)
                    problemReporter().parseErrorInsertAfterToken(errorStart, errorEnd, currentKind, errorTokenSource,
                                                                 errorTokenName, name);
                break;
            case DELETION_CODE:
                if (this.recoveryScanner != null) {
                ...
                if (this.reportProblem)
                    problemReporter().parseErrorDeleteToken(errorStart, errorEnd, currentKind, errorTokenSource,
                                                            errorTokenName);
                break;
            case INVALID_CODE:
                if (name.length() == 0) {
                    ...
                    if (this.reportProblem)
                        problemReporter().parseErrorReplaceToken(errorStart, errorEnd, currentKind, errorTokenSource,
                                                                 errorTokenName, name);
                } else {
                    if (this.recoveryScanner != null) {
                        if (addedToken > -1) {
                    ...
                    if (this.reportProblem)
                        problemReporter().parseErrorInvalidToken(errorStart, errorEnd, currentKind, errorTokenSource,
                                                                 errorTokenName, name);
                }
                break;
            case SUBSTITUTION_CODE:
            ...
            default:
                if (name.length() == 0) {
                    ...
                    if (this.reportProblem)
                        problemReporter().parseErrorNoSuggestion(errorStart, errorEnd, currentKind, errorTokenSource,
                                                                 errorTokenName);
                } else {
                    if (this.recoveryScanner != null) {
                        if (addedToken > -1) {
