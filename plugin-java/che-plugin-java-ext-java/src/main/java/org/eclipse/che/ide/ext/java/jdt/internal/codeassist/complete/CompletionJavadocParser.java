...
public class CompletionJavadocParser extends JavadocParser {
    ...
    protected Object createArgumentReference(char[] name, int dim, boolean isVarargs, Object typeRef,
        ...
        boolean inCompletion = (refStart <= this.cursorLocation && this.cursorLocation <= refEnd) // completion cursor is between
                               // first and last stacked
                               // identifiers
    ...
    protected Object createFieldReference(Object receiver) throws InvalidInputException {
        ...
        boolean inCompletion = (refStart <= (this.cursorLocation + 1) && this.cursorLocation <= refEnd) // completion cursor is
                               // between first and last
                               // stacked identifiers
                               || ((refStart == (refEnd + 1) && refEnd == this.cursorLocation)) // or it's a completion on empty token
                               || (this.memberStart ==
                                   this.cursorLocation); // or it's a completion just after the member separator with an identifier
        // after the cursor
        if (inCompletion) {
    ...
    /*
     * Verify if method identifier positions include completion location. If so, create method reference and store it. Otherwise
     * return null as we do not need this reference.
     */
    ...
    protected Object createTypeReference(int primitiveToken) {
        ...
        if (nbIdentifiers == 1) { // Single Type ref
            this.completionNode =
                    new CompletionOnJavadocSingleTypeReference(this.identifierStack[this.identifierPtr],
                                                               this.identifierPositionStack[this.identifierPtr], this.tagSourceStart,
                                                               this.tagSourceEnd);
            ...
            for (int i = startPtr; i < this.identifierPtr; i++) {
                ...
                if (start <= this.cursorLocation && this.cursorLocation <= end) {
                    if (i == startPtr) {
                        this.completionNode =
                                new CompletionOnJavadocSingleTypeReference(this.identifierStack[startPtr],
                                                                           this.identifierPositionStack[startPtr], this.tagSourceStart,
                                                                           this.tagSourceEnd);
                    } else {
                        ...
                        this.completionNode =
                                new CompletionOnJavadocQualifiedTypeReference(tokens, this.identifierStack[i], positions,
            ...
            if (this.completionNode == null) {
                ...
                this.completionNode =
                        new CompletionOnJavadocQualifiedTypeReference(tokens, this.identifierStack[this.identifierPtr],
    ...
    private char[][][] possibleTags(char[] prefix, boolean newLine) {
        ...
        if (newLine) {
            System.arraycopy(this.levelTags[BLOCK_IDX], 0, possibleTags[BLOCK_IDX] =
                    new char[this.levelTagsLength[BLOCK_IDX]][], 0, this.levelTagsLength[BLOCK_IDX]);
        } else {
            possibleTags[BLOCK_IDX] = CharOperation.NO_CHAR_CHAR;
        }
        System.arraycopy(this.levelTags[INLINE_IDX], 0, possibleTags[INLINE_IDX] =
                new char[this.levelTagsLength[INLINE_IDX]][], 0, this.levelTagsLength[INLINE_IDX]);
    ...
    private void initLevelTags() {
        ...
        if (this.levelTagsLength[BLOCK_IDX] < BLOCK_ALL_TAGS_LENGTH) {
            System.arraycopy(this.levelTags[BLOCK_IDX], 0, this.levelTags[BLOCK_IDX] =
                    new char[this.levelTagsLength[BLOCK_IDX]][], 0, this.levelTagsLength[BLOCK_IDX]);
        }
        // Init inline tags
        ...
        if (this.levelTagsLength[INLINE_IDX] < INLINE_ALL_TAGS_LENGTH) {
            System.arraycopy(this.levelTags[INLINE_IDX], 0, this.levelTags[INLINE_IDX] =
                    new char[this.levelTagsLength[INLINE_IDX]][], 0, this.levelTagsLength[INLINE_IDX]);
        }
    }
    ...
    protected Object parseArguments(Object receiver) throws InvalidInputException {
        ...
        while (this.index < this.scanner.eofPosition) {
            ...
            if (this.index >= this.scanner.eofPosition) {
                ...
                Object argument =
                        createArgumentReference(this.scanner.getCurrentIdentifierSource(), 0, false, typeRef, null,
                                                (((long)argumentStart) << 32) + this.tokenPreviousPosition - 1);
            ...
            if (readToken() == TerminalTokens.TokenNameIdentifier) {
                ...
                argNamePos =
                        (((long)this.scanner.getCurrentTokenStartPosition()) << 32) + this.scanner.getCurrentTokenEndPosition();
                iToken++;
    ...
    protected boolean parseParam() throws InvalidInputException {
        ...
        if (this.identifierPtr >= 0) {
            ...
            switch (this.identifierPtr) {
                case 2:
                    if (!valid && completionScanner.completionIdentifier != null
        ...
        boolean inCompletion = (startPosition <= (this.cursorLocation + 1) && this.cursorLocation <= endPosition) // completion
                               ...
                               || ((startPosition == (endPosition + 1) &&
                                    endPosition == this.cursorLocation)); // or it's a completion on empty token
        if (inCompletion) {
            if (this.completionNode == null) {
                if (isTypeParam) {
                    this.completionNode =
    ...
    protected boolean parseTag(int previousPosition) throws InvalidInputException {
        ...
        boolean inCompletion =
                (this.tagSourceStart <= (this.cursorLocation + 1) && this.cursorLocation <= this.tagSourceEnd) // completion cursor is
                // between first and last
                // stacked identifiers
                || ((this.tagSourceStart == (this.tagSourceEnd + 1) && this.tagSourceEnd == this.cursorLocation)); // or it's a
        // completion on
        // empty token
        if (inCompletion) {
    ...
    /**
     * Push text. If it includes completion location, then rescan line to see if there's a possible reference under the cursor
     * location.
     *
                    ...
                    switch (token) {
                        ...
                        case TerminalTokens.TokenNameERROR:
                            ...
                            if (this.scanner.currentCharacter == '#') { // @see ...#member
                                ...
                                if (this.completionNode != null) {
                                    int flags =
                                            this.inlineTagStarted ? 0 : CompletionOnJavadoc.TEXT
                                                                        | CompletionOnJavadoc.ONLY_INLINE_TAG;
                        ...
                        case TerminalTokens.TokenNameAT:
                            ...
                            try {
                              ...
                              /*
                               * May be to replace invalid @value tag inside text? if (this.completionNode instanceof
                               * CompletionOnJavadocSingleTypeReference) { CompletionOnJavadocSingleTypeReference
                               * singleTypeReference = (CompletionOnJavadocSingleTypeReference) this.completionNode;
                               * singleTypeReference.tagSourceStart = startPosition; switch (this.tagValue) { case
                               * TAG_VALUE_VALUE: // singleTypeReference.completionFlags |= ONLY_INLINE_TAG; if (this.sourceLevel
                               * < ClassFileConstants.JDK1_5) singleTypeReference.completionFlags |= REPLACE_TAG; break; } } else
                               * if (this.completionNode instanceof CompletionOnJavadocQualifiedTypeReference) {
                               * CompletionOnJavadocQualifiedTypeReference qualifiedTypeRef =
                               * (CompletionOnJavadocQualifiedTypeReference) this.completionNode; qualifiedTypeRef.tagSourceStart
                               * = startPosition; switch (this.tagValue) { case TAG_VALUE_VALUE:
                               * singleTypeReference.completionFlags |= ONLY_INLINE_TAG; if (this.sourceLevel <
                               * ClassFileConstants.JDK1_5) qualifiedTypeRef.completionFlags |= REPLACE_TAG; break; } } //
                               */
                                    } else {
                              /*
                               * May be to replace non-inline tag inside text? if (this.completionNode instanceof
                               * CompletionOnJavadocSingleTypeReference) { CompletionOnJavadocSingleTypeReference
                               * singleTypeReference = (CompletionOnJavadocSingleTypeReference) this.completionNode;
                               * singleTypeReference.tagSourceStart = startPosition; switch (this.tagValue) { case TAG_LINK_VALUE:
                               * case TAG_LINKPLAIN_VALUE: singleTypeReference.completionFlags |= ONLY_INLINE_TAG; case
                               * TAG_SEE_VALUE: singleTypeReference.completionFlags |= REPLACE_TAG; break; } } else if
                               * (this.completionNode instanceof CompletionOnJavadocQualifiedTypeReference) {
                               * CompletionOnJavadocQualifiedTypeReference qualifiedTypeRef =
                               * (CompletionOnJavadocQualifiedTypeReference) this.completionNode; qualifiedTypeRef.tagSourceStart
                               * = startPosition; switch (this.tagValue) { case TAG_LINK_VALUE: case TAG_LINKPLAIN_VALUE:
                               * qualifiedTypeRef.completionFlags |= ONLY_INLINE_TAG; case TAG_SEE_VALUE:
                               * qualifiedTypeRef.completionFlags |= REPLACE_TAG; break; } } //
                               */
                                    }
                                }
                            } catch (InvalidInputException e) {
