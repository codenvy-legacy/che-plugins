...
public class CommentRecorderParser extends Parser {
    ...
    public int flushCommentsDefinedPriorTo(int position) {
        ...
        if (validCount > 0) {
            ...
            while (index < lastCommentIndex && (immediateCommentEnd = -this.scanner.commentStops[index + 1]) >
                                               0) { // only tolerating non-javadoc comments (non-javadoc comment end positions are negative)
                // is there any line break until the end of the immediate comment ? (thus only tolerating line comment)
                immediateCommentEnd--; // comment end in one char too far
                if (org.eclipse.che.ide.ext.java.jdt.internal.compiler.util.Util.getLineNumber(position, this.scanner.lineEnds, 0,
                                                                                          this.scanner.linePtr) !=
                    org.eclipse.che.ide.ext.java.jdt.internal.compiler.util.Util.getLineNumber(
    ...
    private void pushOnCommentsStack(int start, int end) {
        ...
        for (int i = start; i <= end; i++) {
            ...
            int commentStart =
                    this.commentPtr == -1 ? -1 : (this.commentStarts[this.commentPtr] < 0
            ...
            if (commentStart == -1 || scannerStart > commentStart) {
                ...
                if (++this.commentPtr >= stackLength) {
                    System.arraycopy(this.commentStarts, 0, this.commentStarts = new int[stackLength + CommentIncrement], 0,
                                     stackLength);
                    System.arraycopy(this.commentStops, 0, this.commentStops = new int[stackLength + CommentIncrement], 0,
                                     stackLength);
                }
