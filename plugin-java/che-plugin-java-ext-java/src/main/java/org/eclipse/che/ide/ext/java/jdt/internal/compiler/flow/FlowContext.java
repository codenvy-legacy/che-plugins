...
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.ast.SubRoutineStatement;
...
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.lookup.CatchParameterBinding;
...
public class FlowContext implements TypeConstants {
     ...
     *         This is for checking exception handlers for exceptions raised during the auto close of
                    ...
                    for (int caughtIndex = 0, caughtCount = caughtExceptions.length; caughtIndex < caughtCount; caughtIndex++) {
                        ...
                        switch (state) {
                            case Scope.EQUAL_OR_MORE_SPECIFIC:
                                exceptionContext.recordHandlingException(caughtException, flowInfo.unconditionalInits(),
                                                                         raisedException, raisedException,
                                                                         // precise exception that will be caught
                            ...
                            case Scope.MORE_GENERIC:
                                exceptionContext.recordHandlingException(caughtException, flowInfo.unconditionalInits(),
                                                                         raisedException, caughtException, location, false);
                        ...
                        for (int raisedIndex = 0; raisedIndex < raisedCount; raisedIndex++) {
                            ...
                            if ((raisedException = raisedExceptions[raisedIndex]) != null) {
                                int state = caughtException == null ? Scope.EQUAL_OR_MORE_SPECIFIC /*
                                                                    ...
                                                                    : Scope.compareTypes(raisedException, caughtException);
                                if (abruptlyExitedLoops != null && caughtException != null && state != Scope.NOT_RELATED) {
                                    for (int i = 0, abruptlyExitedLoopsCount = abruptlyExitedLoops.size(); i < abruptlyExitedLoopsCount;
                                         i++) {
                                        LoopingFlowContext loop = (LoopingFlowContext)abruptlyExitedLoops.get(i);
                                        loop.recordCatchContextOfEscapingException(exceptionContext, caughtException);
                                    }
    ...
    public FlowInfo getInitsForFinalBlankInitializationCheck(TypeBinding declaringType, FlowInfo flowInfo) {
        ...
        do {
            if (current instanceof InitializationFlowContext) {
                ...
                current =
                        exceptionContext.initializationParent == null ? exceptionContext.parent
     ...
     *
     * @param local
     *         the local variable involved in the check
     * @param expression
     *         the expression within which local lays
     * @param status
     *         the status against which the check must be performed; one of {@link #CAN_ONLY_NULL CAN_ONLY_NULL},
     ...
     *
     * @param scope
     *         the scope into which the check is performed
     * @param local
     *         the local variable involved in the check
     * @param reference
     *         the expression within which local lies
     * @param checkType
     *         the status against which the check must be performed; one of {@link #CAN_ONLY_NULL CAN_ONLY_NULL},
     ...
     *         {@link #IN_INSTANCEOF})
     * @param flowInfo
     *         the flow info at the check point; deferring contexts will perform supplementary checks against flow info
