 ...
 *     Stephan Herrmann <stephan@cs.tu-berlin.de> - Contribution for bug 282152 - [1.5][compiler] Generics code rejected by Eclipse but
 *     accepted by javac
 *******************************************************************************/
...
public class TypeVariableBinding extends ReferenceBinding {
    ...
    public int boundCheck(Substitution substitution, TypeBinding argumentType) {
            ...
            switch (wildcard.boundKind) {
                case Wildcard.EXTENDS:
                    ...
                    for (int i = 0, length = this.superInterfaces.length; i < length; i++) {
                        TypeBinding substitutedSuperType =
                                hasSubstitution ? Scope.substitute(substitution, this.superInterfaces[i])
        ...
        for (int i = 0, length = this.superInterfaces.length; i < length; i++) {
            TypeBinding substitutedSuperType =
                    hasSubstitution ? Scope.substitute(substitution, this.superInterfaces[i]) : this.superInterfaces[i];
            if (substitutedSuperType != argumentType) {
                if (!argumentType.isCompatibleWith(substitutedSuperType)) {
                    return TypeConstants.MISMATCH;
                }
    ...
    /** @see org.eclipse.che.ide.ext.java.jdt.internal.compiler.lookup.ReferenceBinding#readableName() */
    ...
    /** @see org.eclipse.che.ide.ext.java.jdt.internal.compiler.lookup.ReferenceBinding#shortReadableName() */
