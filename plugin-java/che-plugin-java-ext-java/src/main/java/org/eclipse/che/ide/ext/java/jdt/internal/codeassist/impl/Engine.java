...
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.lookup.ImportConflictBinding;
...
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.parser.SourceTypeConverter;
...
public abstract class Engine implements ITypeRequestor {
    ...
    /**
     * Add additional source types (the first one is the requested type, the rest is formed by the secondary types defined in the
     * same compilation unit).
     */
    public void accept(ISourceType[] sourceTypes, PackageBinding packageBinding, AccessRestriction accessRestriction) {
        CompilationResult result =
                new CompilationResult(sourceTypes[0].getFileName(), 1, 1, this.compilerOptions.maxProblemsPerUnit);
        CompilationUnitDeclaration unit = SourceTypeConverter.buildCompilationUnit(sourceTypes,// sourceTypes[0] is always toplevel
                                                                                   // here
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (importBinding.onDemand) {
                ...
                if (!(importBinding.resolvedImport instanceof MethodBinding)
                    ...
                    this.importsCache[this.importCacheCount++] =
                            new char[][]{importBinding.compoundName[importBinding.compoundName.length - 1],
        ...
        for (int i = 0; i < this.importCacheCount; i++) {
            ...
            if (CharOperation.equals(typeName, importName[0])) {
                char[] fullyQualifiedTypeName =
                        enclosingTypeNames == null || enclosingTypeNames.length == 0 ? CharOperation.concat(packageName,
                                                                                                            typeName, '.') : CharOperation
                                .concat(CharOperation.concat(packageName, enclosingTypeNames, '.'),
                                        typeName, '.');
...
//
//                     // TODO use nameEnvironment implementation
...
//                      }
    ...
    /*
     * Find the node (a field, a method or an initializer) at the given position and parse its block statements if it is a method
     * or an initializer. Returns the node or null if not found
     */
