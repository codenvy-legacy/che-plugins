...
public class MissingTypesGuesser extends ASTVisitor {
    ...
    private boolean computeMissingElements(QualifiedTypeReference[] substituedTypeNodes, char[][][] originalTypeNames,
        ...
        if (typeRef.resolvedType != null) {
            if (typeRef.resolvedType.isValidBinding()) {
                ArrayQualifiedTypeReference convertedType =
                        new ArrayQualifiedTypeReference(typeRef.tokens, typeRef.dimensions(), typeRef.sourcePositions);
                ...
                if (typeNames == null || typeNames.length == 0)
                    return null;
                ArrayQualifiedTypeReference convertedType =
                        new ArrayQualifiedTypeReference(typeNames[0], typeRef.dimensions(), new long[typeNames[0].length]);
                convertedType.sourceStart = typeRef.sourceStart;
        ...
        if (typeRef.resolvedType != null) {
            ...
            if (typeRef.resolvedType.isValidBinding()) {
                ParameterizedQualifiedTypeReference convertedType =
                        new ParameterizedQualifiedTypeReference(typeRef.tokens, convertedTypeArguments, typeRef.dimensions(),
                ...
                ParameterizedQualifiedTypeReference convertedType =
                        new ParameterizedQualifiedTypeReference(typeNames[0], newConvertedTypeArguments, typeRef.dimensions(),
        ...
        if (typeRef.resolvedType != null) {
            ...
            if (typeRef.resolvedType.isValidBinding()) {
                ParameterizedSingleTypeReference convertedType =
                        new ParameterizedSingleTypeReference(typeRef.token, convertedTypeArguments, typeRef.dimensions, 0);
                convertedType.sourceStart = typeRef.sourceStart;
                convertedType.sourceEnd = typeRef.sourceEnd;
                return convertedType;
            } else if ((typeRef.resolvedType.problemId() & ProblemReasons.NotFound) != 0) {
                ...
                ParameterizedQualifiedTypeReference convertedType =
                        new ParameterizedQualifiedTypeReference(typeNames[0], allConvertedTypeArguments, typeRef.dimensions,
                                                                new long[typeNames[0].length]);
                convertedType.sourceStart = typeRef.sourceStart;
                convertedType.sourceEnd = typeRef.sourceEnd;
                this.substituedTypes.put(convertedType, typeNames);
    ...
    private TypeReference convert(QualifiedTypeReference typeRef) {
        if (typeRef.resolvedType != null) {
            if (typeRef.resolvedType.isValidBinding()) {
                ...
                if (typeNames == null || typeNames.length == 0)
                    return null;
                QualifiedTypeReference convertedType =
                        new QualifiedTypeReference(typeNames[0], new long[typeNames[0].length]);
                convertedType.sourceStart = typeRef.sourceStart;
                convertedType.sourceEnd = (int)(typeRef.sourcePositions[0] & 0x00000000FFFFFFFFL);
                this.substituedTypes.put(convertedType, typeNames);
                this.originalTypes.put(convertedType, typeName);
        ...
        ISearchRequestor storage = new ISearchRequestor() {
            ...
            public void acceptType(char[] packageName, char[] typeName, char[][] enclosingTypeNames, int modifiers,
                ...
                char[] fullyQualifiedName =
                        CharOperation.concat(packageName,
                                             CharOperation.concat(CharOperation.concatWith(enclosingTypeNames, '.'), typeName, '.'), '.');
        ...
        for (int i = 0; i < this.combinationsCount; i++) {
            ...
            if (!this.problemFactory.hasForbiddenProblems) {
                if (guessedType != null) {
                    ...
                    if (computeMissingElements(substituedTypeNodes, originalTypeNames, missingElements,
                                               missingElementsStarts, missingElementsEnds)) {
                        requestor.accept(guessedType.capture(scope, typeRef.sourceEnd), missingElements,
                                         missingElementsStarts, missingElementsEnds, this.problemFactory.hasAllowedProblems);
                    }
                }
            }
        }
    ...
    private void nextSubstitution(QualifiedTypeReference[] substituedTypeNodes, char[][][][] subtitutions,
