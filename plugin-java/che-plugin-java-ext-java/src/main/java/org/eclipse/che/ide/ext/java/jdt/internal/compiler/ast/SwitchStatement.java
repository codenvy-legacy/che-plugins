...
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.flow.SwitchFlowContext;
...
import org.eclipse.che.ide.ext.java.jdt.internal.compiler.lookup.SyntheticMethodBinding;
...
public class SwitchStatement extends Statement {
            ...
            if (this.statements != null) {
                ...
                for (int i = 0, max = this.statements.length; i < max; i++) {
                    ...
                    if ((caseIndex < this.caseCount) && (statement == this.cases[caseIndex])) { // statement is a case
                        ...
                        caseIndex++;
                        if (fallThroughState == FALLTHROUGH && (statement.bits & ASTNode.DocumentedFallthrough) ==
                                                               0) { // the case is not fall-through protected by a line comment
                    ...
                    if ((complaintLevel = statement.complainIfUnreachable(caseInits, this.scope, complaintLevel)) <
    ...
    public void generateCodeForStringSwitch(BlockScope currentScope) {
        ...
        try {
            ...
            StringSwitchCase[] stringCases =
                    new StringSwitchCase[this.caseCount]; // may have to shrink later if multiple strings hash to same code.
