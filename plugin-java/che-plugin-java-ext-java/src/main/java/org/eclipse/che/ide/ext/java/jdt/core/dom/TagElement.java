...
public final class TagElement extends ASTNode implements IDocElement {
     ...
     * @since 3.0
     */
    public static final SimplePropertyDescriptor TAG_NAME_PROPERTY = new SimplePropertyDescriptor(TagElement.class,
                                                                                                  "tagName", String.class, OPTIONAL);
    //$NON-NLS-1$
    ...
    public static final ChildListPropertyDescriptor FRAGMENTS_PROPERTY = new ChildListPropertyDescriptor(
            TagElement.class, "fragments", IDocElement.class, CYCLE_RISK); //$NON-NLS-1$
    ...
    /** The list of doc elements (element type: {@link IDocElement}). Defaults to an empty list. */
    private ASTNode.NodeList fragments = new ASTNode.NodeList(FRAGMENTS_PROPERTY);

    /**
     * Creates a new AST node for a tag element owned by the given AST. The new node has no name and an empty list of fragments.
     * <p>
    ...
    /**
     * Returns this node's tag name, or <code>null</code> if none. For top level doc tags such as parameter tags, the tag name
     * includes the "@" character ("@param"). For inline doc tags such as link tags, the tag name includes the "@" character
     * ("@link"). The tag name may also be <code>null</code>; this is used to represent the material at the start of a doc comment
    ...
    /**
     * Sets the tag name of this node to the given value. For top level doc tags such as parameter tags, the tag name includes the
     * "@" character ("@param"). For inline doc tags such as link tags, the tag name includes the "@" character ("@link"). The tag
     * name may also be <code>null</code>; this is used to represent the material at the start of a doc comment preceding the first
     * explicit tag.
     *
     * @param tagName
     *         the tag name, or <code>null</code> if none
     */
     ...
     * line breaks (and leading whitespace on new lines, including any leading "*").
     * {@link org.eclipse.che.ide.ext.java.jdt.core.dom.TagElement} nodes are used to represent tag elements (e.g., "@link") nested within
     * this tag element.
     * </p>
     * <p>
     * Here are some typical examples:
     * <ul>
     * <li>"@see Foo#bar()" - TagElement with tag name "@see"; fragments() contains a single MethodRef node</li>
     * <li>"@param args the program arguments" - TagElement with tag name "@param"; 2 fragments: SimpleName ("args"), TextElement
     * (" the program arguments")</li>
     * <li>"@return See {&#64;link #foo foo} instead." - TagElement with tag name "@return"; 3 fragments: TextElement ("See "),
     * TagElement (for "&#64;link #foo foo"), TextElement (" instead.")</li>
     * </ul>
     * The use of Name, MethodRef, and MemberRef nodes within tag elements allows these fragments to be queried for binding
     * information.
     * </p>
     * <p/>
     * Adding and removing nodes from this list affects this node dynamically. The nodes in this list may be of various types,
     * including {@link TextElement}, {@link org.eclipse.che.ide.ext.java.jdt.core.dom.TagElement}, {@link Name}, {@link MemberRef}, and
     * {@link MethodRef}. Clients should assume that the list of types may grow in the future, and write their code to deal with
     * unexpected nodes types. However, attempts to add a non-proscribed type of node will trigger an exception.
     *
    ...
    /**
     * Returns whether this tag element is nested within another tag element. Nested tag elements appears enclosed in "{" and "}";
     * certain doc tags, including "@link" and "@linkplain" are only meaningful as nested tags. Top-level (i.e., non-nested) doc
     * tags begin on a new line; certain doc tags, including "@param" and "@see" are only meaningful as top-level tags.
     * <p>
     * This convenience methods checks to see whether the parent of this node is of type
     * {@link org.eclipse.che.ide.ext.java.jdt.core.dom.TagElement}.
     * </p>
     *
