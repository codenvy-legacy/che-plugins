...
public class FormatJavadocBlock extends FormatJavadocNode implements IJavaDocTagConstants {
    ...
    private void addNode(FormatJavadocNode node) {
        ...
        if (++this.nodesPtr == 0) {
            ...
            System.arraycopy(this.nodes, 0, this.nodes = new FormatJavadocNode[this.nodes.length + INCREMENT_ARRAY_SIZE],
                             0, this.nodesPtr);
        }
        ...
        while (lastNode.isText()) {
            ...
            if (lastText.depth == htmlDepth || // found same html tag level => use it
                ...
                if (lastText.isHtmlTag()) {
                    ...
                    if (text != null && text.isHtmlTag() && !text.isClosingHtmlTag()
                        && text.getHtmlTagIndex() == lastText.getHtmlTagIndex() && !lastText.isClosingHtmlTag()) {
                        lastText.closeTag();
